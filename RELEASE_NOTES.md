...
- **Scripts:** [`scripts/144_md_repo_current_state_summary.py`](scripts/144_md_repo_current_state_summary.py) extended (core analytic row counts, MRQ total + pending, manifest mismatch warnings); [`scripts/119_md_formalization_validate.py`](scripts/119_md_formalization_validate.py) Check **11b** (release-mode verified-layer integrity); [`scripts/125_master_verified_views.py`](scripts/125_master_verified_views.py) adds `review_grain` / `review_status_source` / `review_join_key`; [`scripts/145_export_release_manifest_pointer.py`](scripts/145_export_release_manifest_pointer.py) refreshes `exports/release_manifests/LATEST_MANIFEST.json` from live.
- **Docs:** README, `docs/REPO_STATUS.md`, `truth_sync_summary.md` aligned; [`exports/release_manifests/README.md`](exports/release_manifests/README.md) + additive fields on checked-in `LATEST_MANIFEST.json` (historical checkpoint unless regenerated).
- **Tests:** `tests/test_source_of_truth_narrative_sync.py`, `tests/test_119_verified_layer_release_integrity.py`; `scripts/check_doc_paths.py` extended.
...
> **Superseded narrative (partial):** MRQ row counts / dual `run_label` stories below are **point-in-time 2026-04-07** — reconcile against **live** `qa.manual_review_queue` and fresh `119 --release-mode` before citing as current. **Canonical framing:** [`docs/final_source_of_truth_contract.md`](docs/final_source_of_truth_contract.md) and [`truth_sync_summary.md`](truth_sync_summary.md).

**Live sign-off outcome (MotherDuck, service-account token):** **Two timestamped `119 --release-mode` outcomes same day:** (1) **BLOCKED** — specimen/FHIR QA diagnostics FAIL (`broken_fhir_refs` large) — `studies/20260407_publication_signoff_live/validation_report.md`. (2) **PASS WITH WARNINGS** — `broken_fhir_refs=0`; specimen-adjacent / genomic review burden WARN — `studies/20260407_live_truth_and_lineage_contract_audit/119_release_validation/validation_report.md`. **Supersession (2026-04-07 PM UTC):** live reaudit [`studies/20260407_live_publication_signoff_reaudit/`](studies/20260407_live_publication_signoff_reaudit/) — MRQ **11,244** rows (two run labels × 5,622), **0** NULL `verification_status`; verification histogram **differs** from the older **5,620 / 5,622** `SYNTHETIC_*` snapshot — **reconcile on live MotherDuck** before manuscript language. **Final institutional non-Tg lab wave** (`final_institutional_20260407`) **ingested** via `127`; longitudinal canonical layer **includes** that wave per [`studies/20260411_final_master_release/EVIDENCE_PACK.md`](studies/20260411_final_master_release/EVIDENCE_PACK.md). **Single top-level repo posture (April 2026, superseded by 2026-04-14 contract):** **Live `main`/`qa` on MotherDuck are canonical** — automation (`119 --release-mode`) can be **PASS WITH WARN** while **governance** (human-reviewed MRQ / promotion where policy requires) remains separate; **cite the dated `119` artifact** for automation state and **do not** treat “missing lab wave” as the current blocker. See [`docs/final_source_of_truth_contract.md`](docs/final_source_of_truth_contract.md).
