...
SELECT
  ...
  LN_TOTAL_EXAMINED, LN_TOTAL_POSITIVE, LN_POSITIVE_FLAG,
  CASE WHEN LN_POSITIVE_FLAG = 1 OR LN_TOTAL_POSITIVE > 0 THEN TRUE
       ELSE FALSE
  END AS LN_POSITIVE,
  SURG_PROCEDURE_TYPE,
  MOLECULAR_TESTED_CONFIRMED, BRAF_POSITIVE_FINAL, RAS_POSITIVE_FINAL,
  ANY_RECURRENCE_FLAG, OVERALL_SURVIVAL_YEARS,
  FIRST_SURGERY_DATE
FROM THYROID_VALIDATION.PUBLIC.CANONICAL_PATIENT_MASTER_FLAT
WHERE IS_MALIGNANT = TRUE AND HISTOLOGY_FINAL IS NOT NULL
"""
