...
SELECT
  cp.RESEARCH_ID,
  cp.AGE_AT_SURGERY, cp.SEX, cp.RACE, cp.BMI_COMBINED,
  cp.HISTOLOGY_FINAL,
  COALESCE(lu.HISTOLOGY_GROUP, 'Other') AS HISTOLOGY_GROUP,
  cp.IS_MALIGNANT, cp.FIRST_SURGERY_DATE,
  cp.AJCC8_STAGE_GROUP, cp.AJCC8_T_STAGE, cp.AJCC8_N_STAGE, cp.AJCC8_M_STAGE,
...
SELECT
  ...
  COALESCE(lu.HISTOLOGY_GROUP, 'Other') AS HISTOLOGY_GROUP,
  cp.IS_MALIGNANT, cp.FIRST_SURGERY_DATE,
  EXTRACT(YEAR FROM cp.FIRST_SURGERY_DATE) AS SURGERY_YEAR,
  CASE
    WHEN EXTRACT(YEAR FROM cp.FIRST_SURGERY_DATE) BETWEEN 1999 AND 2005 THEN 'Early (1999-2005)'
    WHEN EXTRACT(YEAR FROM cp.FIRST_SURGERY_DATE) BETWEEN 2006 AND 2013 THEN 'Middle (2006-2013)'
    WHEN EXTRACT(YEAR FROM cp.FIRST_SURGERY_DATE) BETWEEN 2014 AND 2019 THEN 'Modern (2014-2019)'
    WHEN EXTRACT(YEAR FROM cp.FIRST_SURGERY_DATE) >= 2020 THEN 'Contemporary (2020+)'
    ELSE 'Unknown'
  END AS ERA,
  cp.TUMOR_SIZE_CM_MAX, cp.AJCC8_STAGE_GROUP, cp.ETE_GRADE,
  cp.SURG_PROCEDURE_TYPE, cp.RAI_RECEIVED_FLAG,
  cp.MOLECULAR_TESTED_CONFIRMED, cp.BRAF_POSITIVE_FINAL,
  cp.ANY_RECURRENCE_FLAG, cp.OVERALL_SURVIVAL_YEARS, cp.FOLLOWUP_YEARS
FROM THYROID_VALIDATION.PUBLIC.CANONICAL_PATIENT_MASTER_FLAT cp
LEFT JOIN THYROID_VALIDATION.PUBLIC.CANONICAL_HISTOLOGY_LOOKUP_V1 lu
  ON cp.HISTOLOGY_FINAL = lu.HISTOLOGY_FINAL_RAW
WHERE cp.FIRST_SURGERY_DATE IS NOT NULL
"""
