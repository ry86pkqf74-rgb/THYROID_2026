...
**LN positivity provenance (`ln_status_source`; mig_258 / CF-mig258-MANUSCRIPT-FILTER-UPDATE).** Lymph-node positivity in this cohort is sourced from a triadic provenance column `ln_status_source ∈ {'staging','count','both',NULL}`, where `staging` indicates AJCC 8 N-stage assertion only, `count` indicates a structured `ln_total_positive > 0` count, and `both` indicates concordance. For analyses requiring numeric LN positivity interpreted consistently with counted metastatic deposits (e.g., sensitivity analyses aligning with LN-burden regression), restricting to `ln_status_source = 'both'` yielded n=1,126 of 4,137 malignant patients in the parent publication spine; descriptive Table 1 and primary models use AJCC N-stage and rollup flags as documented above.
