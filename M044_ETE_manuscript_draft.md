...
- **No/negative ETE** (`ete_grade_final = 'no_negative'`; v1.0 vocabulary 'false'/'absent'/'none' normalized by mig_315, n=173 in strict-DTC analytic frame).
- **Microscopic ETE** (`ete_grade_final = 'microscopic'`, n=2,413 in strict-DTC analytic frame).
- **Gross ETE** (`ete_grade_final = 'gross'`, n=1,241 in strict-DTC analytic frame).
...
Demographic variables: age at first surgery (continuous), sex (female reference). Tumor characteristics: tumor size in cm (continuous); AJCC 8 T/N/stage group for descriptive Table 1.

**LN positivity provenance (`ln_status_source`; mig_258 / CF-mig258-MANUSCRIPT-FILTER-UPDATE).** Lymph-node positivity in this cohort is sourced from a triadic provenance column `ln_status_source ∈ {'staging','count','both',NULL}`, where `staging` indicates AJCC 8 N-stage assertion only, `count` indicates a structured `ln_total_positive > 0` count, and `both` indicates concordance between staging assertion and structured counts. For analyses requiring numeric LN positivity together with enumerated positive nodes (LN-burden sensitivity analyses aligned with mig_258/259), restricting to malignant patients with `ln_status_source = 'both'` yields **n=2,628** (among them **1,126 LN-positive**, 42.8%) versus **n=4,113** malignant patients overall with **1,126** LN-positive on the unrestricted publication spine (`snowflake_trial/reports/m037_sensitivity_ln_both.md`). Descriptive Table 1 and primary models in this manuscript use AJCC N-stage and rollup-derived flags unless a sensitivity specifies the `both` subset.

**AJCC stage group on `canonical_patient_master` (mig_263 Option B).** The column `ajcc8_stage_group` collapses published AJCC 8th edition labels {IVA, IVB, IVC} into a single rollup value `IVB`, including all distant-disease (M1) thyroid carcinomas regardless of histology (ATC, MTC, PDTC). Textbook-published granularity remains in `ajcc8_stage_group_resolved` where analyses require IVA/IVB/IVC as recorded before patient-level rollup.

**Surveillance LN flag (mig_262).** The patient-level suspicious cervical lymph-node flag (`any_suspicious_us_ln_ever`) was rebuilt from per-nodule `canonical_us_thyroid_gland_v2` ultrasound staging: the rebuilt definition identifies 1,733 patients with suspicious LN findings among the ultrasound-eligible set (versus 8 patients under the legacy threshold definition), consistent with operative-referral biology. Cross-cohort contrasts using this flag should cite the mig_262 rebuild.

**Histology in strict-DTC multivariable models** uses five malignant categories with papillary thyroid carcinoma as reference: FTC (`histology_final = follicular carcinoma`), metastatic/recurrent PTC (strings beginning `metastatic PTC` or `recurrent/metastatic PTC`), poorly differentiated DTC (strings containing “poorly differentiated”), high-grade differentiated carcinoma, and residual explicit PTC rows. **Radioactive iodine is not included as a primary covariate** because receipt reflects confounding-by-indication; a parallel model retaining RAI is reported as sensitivity. Pathology covariates of primary interest are lymphatic invasion (`lvi_clean`: extensive, present, focal, indeterminate, missing) and vascular/angioinvasion (`vasc_clean`: extensive, focal, present_ungraded, indeterminate, missing). Lymph-node burden was sourced from `manuscript_workspace.ln_master_rollup_v1`, pre-aggregated to one row per patient by selecting the maximum value for each rollup metric across same-patient records, which yielded central- and lateral-compartment positivity flags. Reoperative context (≥2 surgeries, days to second surgery, completion-thyroidectomy reason) was sourced from `manuscript_workspace.cohort_m040_reoperative_v1` and used in the strict-DTC no/negative ETE subgroup regression (Supplement).

**ETE and synoptic pathology normalization (mig_261).** Extrathyroidal extension and related CAP-template fields were normalized via case- and whitespace-insensitive mapping across `path_synoptics`: six typo variants in `tumor_1_lymphatic_invasion` (for example “preesent”, “indeeterminate”) and two in `tumor_1_extrathyroidal_extension` (“extesive”) were corrected. `path_synoptics.surg_date` was stored as DATE (calendar-only clinical dates) consistent with institutional retype rules.
...
**NLP vs structured pathology signals (mig_265 / CF-mig260b,c,d / mig_261c,d,e).** Several NLP-derived patient-level signals are intentionally under-complete relative to structured pathology extracts: vascular-invasion NLP mentions (`nlp_path_vasc_inv_mentioned`) miss **749** patients flagged vasc-positive in `canonical_invasion_events_v1` (CAP-template patterns recovered in mig_177/179 that NLP did not surface). LN positivity encoded in PMH/NLP rollups disagrees with `canonical_path_malignant_events_v1.ln_involved` in **1,105** patients. Smoking status is documented for 27 of 10,871 patients (0.25%)—far below expected clinical prevalence. Family history of thyroid cancer (30 patients) and family history of any cancer (16 patients) extract at \<1% vs 5–15% expected in oncology referral populations. Subgroup analyses treating smoking, family history, or vascular invasion as primary exposures require cross-validation against `canonical_invasion_events_v1` (vascular) or `canonical_path_malignant_events_v1.ln_involved` (LN) before primary causal claims.

**NLP comorbidity and social history.** Smoking status, family history of thyroid malignancy, family history of other cancers, and osteoporosis were not used as primary covariates in this manuscript. Where such fields are drawn from the canonical PMH rollup, note that NLP extraction of preoperative notes recovers these signals at documented cohort coverage well below expected community rates for an operative population (sub‑percent for smoking and family history in the validation round underlying migration `mig_265`). Subgroup or stratified analyses by smoking, family history, or bone health would therefore be underpowered and were not pursued. The PMH “definitive” phenotype tier was expanded in `mig_265` to include `probable` evidence strength alongside literal `definitive` (`CF-mig261c/d/e` narrative coverage). Future structured social-history and family-history extraction is tracked as `CF-mig265-NLP-SOCHX-FAMHX-REFRESH-SCOPE`.
...
Gross vs microscopic ETE: adjusted OR 1.77 (95% CI 1.15–2.71; p=0.009)

No/negative vs microscopic ETE: adjusted OR 0.55 (95% CI 0.23–1.32; p=0.183)

**Sensitivity — strict-DTC with RAI covariate retained:**

Strict cohort — gross vs microscopic ETE (RAI covariate retained): adjusted OR 1.33 (95% CI 0.88–2.01; p=0.1698)

Strict cohort — no/negative vs microscopic ETE (RAI covariate retained): adjusted OR 0.51 (95% CI 0.21–1.24; p=0.136)

(McFadden pseudo-R²=0.1460; n=3619, path-proven events=136; likelihood-ratio χ²=169.23 vs intercept-only). A Cox proportional hazards model on the same strict-DTC subset (documented surgery date, positive follow-up; **no RAI covariate**; n=2431) estimated HR=1.34 (95% CI 0.91–1.98; p=0.1411) for gross vs microscopic ETE.

Global **ETE × AJCC8 N-stage interaction** (likelihood ratio vs main-effects-only model): LR χ²=5.73, df=6, p=0.4546. The omnibus interaction test did not reach α=0.05; stratum-specific contrasts are nevertheless presented given clinically heterogeneous crude gradients (especially within N1b).

**Within-N-stage gross-vs-microscopic contrasts** (same adjustment bundle excluding the fixed stratum’s N-stage factor):
- **N0:** adjusted OR 1.69 (95% CI 0.73–3.90; p=0.2204); n=1083, path-proven events=28.
- **N1a:** adjusted OR 1.82 (95% CI 1.12–2.95; p=0.01522); n=2337, path-proven events=91.
- **N1b:** adjusted OR 1.21 (95% CI 0.07–20.87; p=0.8963); n=74, path-proven events=16.
...
Second, the no/negative ETE group requires careful interpretation regarding both its adjusted estimate and its absolute recurrence rate. The adjusted estimate for no/negative versus microscopic ETE reversed direction between the v1.0 and corrected analyses (aOR 2.72 → 0.55 vs microscopic ETE), driven entirely by the tripled no/negative denominator (68 → 173 patients) after mig_315 normalized Boolean vocabulary artifacts ('false'/'absent'/'none') to the canonical 'no_negative' label. Both estimates are statistically non-significant (p > 0.10) with wide confidence intervals that include 1.0; neither supports a directional biological claim about the no/negative group. The corrected group also contains an ascertainment-driven subgroup: the 173 no/negative ETE patients are enriched for clinically apparent lateral-neck disease at presentation, and the observed recurrence signal in the crude OR (2.75, 95% CI 1.38–5.50) is substantially attenuated after multivariable adjustment. The qualitative conclusion remains unchanged: microscopic and gross ETE are independently associated with recurrence risk in the adjusted model, while the no/negative ETE group is a heterogeneous category that should not be treated as a direct biologic comparator without propensity-matching on tumor size, N stage, and surgical complexity.
...
Limitations. This is a retrospective single-institution cohort with substantial follow-up censoring (33.9% of patients have zero follow-up), 0.0% missing surgery-date indicator (`surg_date_missing`; n=0/3619). Two post-package data-quality migrations affected the analytic cohort: mig_313 corrected an M-stage corruption in which `distant_mets_proxy = recurrence_flag` artificially inflated Stage IVB prevalence from 816 to 76 patients (a 91% reduction), and mig_315 normalized the ETE-grade vocabulary ('false'/'absent'/'none' → 'no_negative'), expanding the no/negative ETE group from 68 to 173 patients. These corrections changed the strict-DTC analytic N from 3,578 to 3,619 and path-proven events from 105 to 136. The primary finding (adjusted OR for gross vs microscopic ETE) was stable across the correction (1.77 in both v1.0 and corrected analyses). and free-text data-quality variability in the lymphatic-invasion field. The path-proven recurrence endpoint depends on completeness of pathology and operative-event capture; under-ascertainment is plausible particularly for patients whose recurrence was managed at outside institutions. The legacy `structural_recurrence_flag` on `canonical_patient_master` (headline M044-cohort counts in `manuscript_workspace.m044_legacy_recurrence_flag_audit_v1`) is inconsistent with the canonical dual-track resolution and was not used as a primary endpoint; its high apparent rate is an artifact and should not be over-interpreted. Multivariable adjustment cannot fully account for the systematic differences between the no/negative ETE subgroup and the rest of the cohort, and we therefore present that subgroup descriptively rather than as a direct biologic comparator. Surgery-flag calendar strata in this analytic extract: 1999‑01‑01–2024‑12‑31 n=3832, pre‑1999 n=3, post‑2024 n=33 (counts from mig_258 `surg_date_*`; not a censorship rule for the primary Cox slice). Era-stratified sensitivity analyses are reported in the supplement.
