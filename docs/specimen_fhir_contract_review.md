...
| Local FHIR NDJSON export — RW attach (`--md`) | `specimen_fhir_export_restore_v1` (`scripts/141_fhir_specimen_json_export.py`; override via `MOTHERDUCK_CUSTOM_USER_AGENT`). **Reviewer-ops live run (2026-04-13):** `specimen_fhir_reviewer_ops_v1` — see `studies/specimen_fhir_reviewer_ops_20260413_165952/report.md` |
...
### Live reviewer-ops path (2026-04-13)

Executed on prod (`Thyroid 2026`): **136** `writer --md-env prod --prefer-sa` → **141** `--md` with `MOTHERDUCK_CUSTOM_USER_AGENT` / `MOTHERDUCK_SESSION_HINT` = `specimen_fhir_reviewer_ops_v1`, `--output-root exports` → `exports/fhir_specimen_20260413_165952/` (10,139 bundles; route `bundle_table`; counts match `main.fhir_bundle_specimen_export_v1`). **Read-scaling** reader refresh + **141** `--read-scaling` were not run in-repo (no `MD_READ_SCALING_TOKEN`); orgs should issue read-scaling or restricted-share access per §*Read scaling for reviewers*. Full command log and manifest excerpt: `studies/specimen_fhir_reviewer_ops_20260413_165952/report.md`.
