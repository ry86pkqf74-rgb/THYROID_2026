...
6. **Route findings to Linear; verify columns with the GCloud AI agent.** When a safeguard finds a violation or a competing-source dependency, file or update a **Linear issue** (workspace `rostemp`, `THY-` prefix) — don't leave it in a comment. As an independent second pass on column choice, run the AI verification: BigQuery `AI.GENERATE` needs a Vertex connection (not provisioned as of 2026-05-14), so use **Vertex AI Gemini directly** — `google-genai`, `Client(vertexai=True, project='thyroid-canonical-pub-2026', location='global')`, model `gemini-2.5-flash` — from the Mac via Desktop Commander, feeding each column + its data-dictionary entry. Reference: `m0XX_ai_column_verification.py`. On M011 the AI agent independently flagged the same two competing-source columns the human-ratified `canonical_column_verification_registry_v1` did — use both; treat the registry as authoritative and the AI pass as the independent concordance check.
