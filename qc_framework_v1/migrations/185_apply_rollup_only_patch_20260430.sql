...
-- =============================================================================
-- Migration 185 — canonical_path_malignant_patient_rollup_v1 rollup-only patch
-- =============================================================================
-- Date:   2026-04-30 (UTC)
-- Author: Logan Glosser <logan.glosser@gmail.com>
--
-- Batch:  mig185_apply_rollup_only_patch_20260430
-- Lane:    mig_185 / apply_rollup_only_patch (option B)
--
...
CREATE TABLE IF NOT EXISTS
    "Thyroid 2026 UPdated".archive_pub_v1_0.canonical_path_malignant_patient_rollup_v1_pre_mig185_rollup_only_patch_20260430
AS
SELECT
    *,
    CAST(CURRENT_TIMESTAMP AS TIMESTAMP) AS pre_mig185_rollup_only_patch_snapshot_ts
...
SELECT
    ...
    'mig185_apply_rollup_only_patch_20260430'::VARCHAR AS build_script,
    CAST(CURRENT_TIMESTAMP AS TIMESTAMP)               AS build_ts
FROM rebuilt r
LEFT JOIN "Thyroid 2026 UPdated".archive_pub_v1_0.canonical_path_malignant_patient_rollup_v1_pre_mig185_rollup_only_patch_20260430 pre
...
SET notes = COALESCE(notes, '')
    || ' | mig_185 rollup-only patch: COUNT(*)→COUNT(DISTINCT(surgery_episode_id,tumor_ordinal)) for n_tumors_total; dominant_histology mode() after one-row-per-grain collapse; events left untouched per source-fidelity; CF-mig185-ROLLUP-GRAIN-DEDUPE.'
WHERE schema_name = 'main'
  AND table_name = 'canonical_path_malignant_patient_rollup_v1'
  AND column_name IN ('n_tumors_total', 'dominant_histology');

-- Optional: register new events column if your registry workflow requires an INSERT;
-- (omitted here — many repos add via separate registry hygiene migration).
...
SELECT
    'mig185_apply_rollup_only_patch_20260430',
...
WHERE NOT EXISTS (
    ...
    WHERE run_id = 'mig185_apply_rollup_only_patch_20260430'
);

COMMIT;

-- -----------------------------------------------------------------------------
-- §F Post-state probes (run after commit; expect inflation cohort → 0)
-- -----------------------------------------------------------------------------
