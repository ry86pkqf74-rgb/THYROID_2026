...
- T3b vs T4a vs T4b collapsed to `indeterminate_t3b_t4a_t4b_requires_llm` when `gross_ete=TRUE` (no structured invasion-target column in path events at migration 04; **2026-04-24**: tier-2 LLM + `main.canonical_t4b_invasion_*_v1` now supply prevertebral / carotid / mediastinal structure — see `project_mig_55_t4b_invasion_closeout.md`).
...
- **2026-04-24 PM** — Prompt 51 / migration 52: `manuscript_workspace.ete_manuscript_analytic_v1` published. Per-event ETE-focused analytic view fusing clean overlays; prerequisite views: `path_malignant_event_fingerprint_v1`, eight `path_malignant_overlay_*_w_fp_v1`, `path_event_discordance_dedup_ete_v1`; `cohort_descriptive_full_cohort_v1` repointed to `main.canonical_us_patient_master_VIEW_v2` (cupm rename). MotherDuck probes: row parity 6,689; PTC 2,710 patients (`COUNT(DISTINCT research_id) WHERE cohort_ptc` = `qc_manuscript_cohort_v2_ptc`); eligible∧`ete_norm` non-null 901 path events; t-stage discordance among eligible 0.44%; zero eligible with null `surgery_episode_id_global`. Deprecation log: `pointer_only` for `MANUSCRIPT_ETE` (NOT NULL on `deprecated_object` uses self-pointer). PATH20/PATH21 log patch previously merged via `52a_path20_21_log_patch.sql`.
- **2026-04-24 PM** — Migration 53 / prompt_52: `ete_llm_grade_patient_v1` + `ete_manuscript_analytic_v2` published. LLM grade layer sourced from existing `main.note_entities_llm_pathology` (711 ETE entities) onto 196 PTC present_unspecified patients. Reclassified 24→gross + 3→microscopic via regex over `entity_value` (strap muscle / trachea / esophagus / RLN / substernal = gross; microscopic / minimal / perithyroidal fat / focal = microscopic). Final PTC analytic-eligible patient buckets: gross 40, microscopic 51, unspec_remaining 167, none 30 — clean-graded N=124. v1 preserved for audit; downstream uses `ete_grade_final` ∈ {gross, microscopic, unspec_remaining, none} with `ete_grade_source` ∈ {structured, llm_subgrade, unresolved}.
- **2026-04-24 PM** — Migration 54 / prompt_53: `ete_llm_fresh_subgrade_patient_v1` + `ete_manuscript_analytic_v3` published. Fresh narrow 4-way LLM pass (gross/microscopic/absent/unable_to_determine) on 132 ETE-bearing OPNOTE/HP/DC_SUM/ED_NOTE snippets from the 167 PTC unspec_remaining patients (86 pts with any candidate note; 81 with zero). Extraction via qwen3:14b on fleet server S8 (ollama 0.18, think=false, num_ctx=6144). Scripts: `scripts/410_ete_subgrade_extract_notes.py` (MotherDuck→local JSONL), `scripts/411_ete_subgrade_llm_run.py` (narrow classifier), `scripts/412_ete_subgrade_load_to_md.py` (results→`main.note_entities_llm_ete_subgrade_v1`). Downstream uses `ete_grade_final_v3` ∈ {gross, microscopic, none, unspec_remaining} with `ete_grade_source_v3` ∈ {structured, llm_subgrade, llm_fresh_subgrade, llm_fresh_absent, llm_unable, unresolved}. Post-run acceptance probe + 81 truly-uncoverable patients (no ETE language in any note) documented in commit notes.
- **2026-04-24 PM** — Migration 56 (`mig_55_t4b_invasion_20260424`): `main.canonical_t4b_invasion_events_v1` (944 rows) + `main.canonical_t4b_invasion_patient_rollup_v1` (434 pts, 10 `any_pT4b_final`) from `note_entities_llm_t4b_invasion_v1`; `canonical_ete_subgrade_patient_rollup_v1` rebuilt with `any_pT4b_from_t4b_invasion` + `pT4b_ete_vs_t4b_invasion_discordant` (non-null only when ETE `any_pT4b` is known). Registry rows for T4b canonicals + refreshed ETE rollup row. Close-out: `project_mig_55_t4b_invasion_closeout.md`.
- **2026-04-30 23:41 EDT** — **Lane LN batch mig_224–229 applied** via `scripts/414_lane_ln_mig_224_229_apply.py`. All 6 migrations executed successfully (provenance rows in `manuscript_workspace.cpm_reconciliation_provenance_v1`). Summary:
  - **mig_224** (`224_histology_vocab_extension_20260430`): +8 typo-fix rows added to `main.histology_vocab_normalization_map_v1` (now 104 rows total — `Follicular caricinoma`, `microcarinoma`, `microcarcinooma`, `microcaricnoma`, `folliucalr`, `follicualr`, `classsical`, `poorly differntiated`). `manuscript_workspace.dim_histology_standardized_VIEW_v1` rebuilt (13 canonical codes). Archive snapshot at `"Thyroid 2026 UPdated".archive_pub_v1_0.histology_vocab_normalization_map_v1_pre_mig224_20260430`.
  - **mig_225** (`225_vw_ln_surgery_publication_safe_20260430`): `manuscript_workspace.vw_ln_surgery_publication_safe_VIEW_v1` created — per-surgery LN publication-safe view (4,008 rows).
  - **mig_226** (`226_vw_ln_patient_publication_safe_20260430`): `manuscript_workspace.vw_ln_patient_publication_safe_VIEW_v1` created — per-patient LN publication-safe rollup (4,008 rows).
  - **mig_227** (`227_vw_ln_histology_attribution_20260430`): `manuscript_workspace.vw_ln_histology_attribution_VIEW_v1` created — LN histology attribution view (5,918 rows).
  - **mig_228** (`228_qc_ln_histology_tables_20260430`): 5 QC tables created — `qc_ln_impossible_counts_v1` (55 rows / impossible LN ratios), `qc_ln_duplicate_rollup_patients_v1` (256 rows / duplicate rollup patients), `qc_ln_multihistology_attribution_queue_v1` (47 rows / attribution ambiguities), `qc_histology_borderline_in_malignant_table_v1` (26 rows / borderline/benign in malignant table), `qc_histology_vocab_typos_v1` (1 row / residual typo).
  - **mig_229** (`229_borderline_quarantine_flag_20260430`): `is_borderline_or_benign_with_staging BOOLEAN` column added to `main.canonical_path_malignant_events_v1` — 27 rows quarantined (TRUE), 6,442 not quarantined (FALSE), total 6,469. Archive snapshot at `"Thyroid 2026 UPdated".archive_pub_v1_0.canonical_path_malignant_events_v1_pre_mig229_quarantine_20260430`. `main.canonical_path_malignant_events_dedup_VIEW_v1` rebuilt (5,944 rows = 6,469 − quarantined 27 − internal dups). `manuscript_workspace.vw_ln_surgery_publication_safe_VIEW_v1` and `manuscript_workspace.vw_ln_histology_attribution_VIEW_v1` recompiled post-quarantine exclusion.
  - Script 414 (`scripts/414_lane_ln_mig_224_229_apply.py`): runner for ordered apply (229 precedes 228 per dependency on quarantine view).
