...
WITH tg_outliers AS (
  SELECT research_id, DATE(lab_datetime) AS event_date,
         CONCAT('tg=', CAST(value_numeric AS STRING), ' ', unit_standardized) AS detail
...
ca_outliers AS (
  SELECT research_id, DATE(lab_datetime) AS event_date,
         CONCAT('ca=', CAST(value_numeric AS STRING), ' ', unit_standardized) AS detail
...
pth_outliers AS (
  SELECT research_id, DATE(lab_datetime) AS event_date,
         CONCAT('pth=', CAST(value_numeric AS STRING), ' ', unit_standardized) AS detail
  FROM `thyroid-canonical-pub-2026.pub_canonical.canonical_labs_pth_v1`
  WHERE value_numeric IS NOT NULL
    AND (value_numeric < 0 OR value_numeric > 5000)
)
SELECT 'qc_lab_sentinels' AS assertion_id, * EXCEPT(detected_at), CURRENT_TIMESTAMP() AS detected_at
FROM (
