#!/usr/bin/env python3
"""mig_98d — Apply Logan-ratified bulk dispositions for seroma.
...
Phases:
  1. Snapshot canonical_complications_events_v1 seroma rows
  2. Compute disposition map (in-memory)
  3. PMH INSERT (before delete) for PMH-attributed pts
  4. DELETE seroma present + suspected rows for non-keep pts
...
def compute_dispositions(con):
    ...
    rids = [r[0] for r in con.execute(
        ...
        f"WHERE complication_type='seroma' AND finding_status IN ('present','suspected')"
    ).fetchall()]
    rid_list = "','".join(rids)
    ...
    timing_q = (
        ...
        f"WHERE c.complication_type='seroma' AND c.finding_status IN ('present','suspected') "
    ...
    struct_q = (
        ...
        f"  WHERE c.complication_type='seroma' AND phen.confirmed_flag=TRUE "
        f"), refined AS ( "
        f"  SELECT DISTINCT research_id FROM {EVENTS} "
        f"  WHERE complication_type='seroma' AND source_table='extracted_complications_refined_v5' "
        f")"
...
def _counts(con):
    ...
    return {
        "present_rows": one(f"SELECT COUNT(*) FROM {EVENTS} WHERE complication_type='seroma' AND finding_status='present'"),
        "present_pts": one(f"SELECT COUNT(DISTINCT research_id) FROM {EVENTS} WHERE complication_type='seroma' AND finding_status='present'"),
        "suspected_rows": one(f"SELECT COUNT(*) FROM {EVENTS} WHERE complication_type='seroma' AND finding_status='suspected'"),
        "absent_rows": one(f"SELECT COUNT(*) FROM {EVENTS} WHERE complication_type='seroma' AND finding_status='absent'"),
        "pmh_seroma": one(f"SELECT COUNT(*) FROM {PMH_EVENTS} WHERE finding_value_norm='seroma'"),
    }


def phase_snapshot(con, do_writes, ts_tag):
    snap = f'{ARCHIVE_DB}.canonical_complications_events_v1_pre98d_seroma_{ts_tag}'
    if do_writes:
        con.execute(f"CREATE TABLE {snap} AS SELECT * FROM {EVENTS} WHERE complication_type='seroma'")
        n = con.execute(f"SELECT COUNT(*) FROM {snap}").fetchone()[0]
        print(f"  snapshot {snap}: {n} rows")
    else:
        n = con.execute(f"SELECT COUNT(*) FROM {EVENTS} WHERE complication_type='seroma'").fetchone()[0]
...
def phase_pmh_insert(con, do_writes, dispositions):
    ...
    for rid, info in pmh_rids:
        existing = con.execute(
            ...
            f"WHERE research_id='{rid}' AND finding_value_norm='seroma'"
        ).fetchone()[0]
        if int(existing) > 0:
            continue

        # Earliest seroma finding_date for this pt
        edt_row = con.execute(
            f"SELECT MIN(finding_date)::VARCHAR FROM {EVENTS} "
            f"WHERE research_id='{rid}' AND complication_type='seroma' "
        ...
        finding_text = (
            f"seroma attributed to {decision_label} ({info['basis']}); "
            f"mig_98d_classifier_logan_curated"
        ).replace("'", "''")
        ...
        src_row_sql = (
            f"CAST(hash('{rid}', 'seroma', CAST('{edt}' AS DATE), 'mig98d_pmh_synthetic') AS VARCHAR)"
        )
        sql = f"""
        ...
        SELECT
          '{rid}', 'mig_98d_pmh_synthetic', {src_row_sql}, 'other_history',
          NULL, 'mig_98d_seroma_classifier_logan_curated',
          '{finding_text}', 'seroma', 'seroma',
          CAST('{edt}' AS DATE), CAST('{edt}' AS DATE),
          'present', 'definitive', NULL, TRUE,
          'mig_98d_classifier_logan_curated', NULL,
          sha256('{sha_input}'),
...
def phase_delete_non_keep(con, do_writes, dispositions):
    ...
    pre_present = con.execute(
        ...
        f"WHERE complication_type='seroma' AND finding_status='present' "
        f"AND research_id NOT IN {keep_sql}"
    ).fetchone()[0]
    pre_susp = con.execute(
        f"SELECT COUNT(*) FROM {EVENTS} "
        f"WHERE complication_type='seroma' AND finding_status='suspected' "
        f"AND research_id NOT IN {keep_sql}"
    ).fetchone()[0]
    if do_writes:
        con.execute(
...
def phase_provenance(con, do_writes):
    run_id = f"mig98d_seroma_{datetime.now(timezone.utc).strftime('%Y%m%d')}"
    if not do_writes:
    ...
    con.execute(
        ...
        VALUES (
            ...
            'mig98d_seroma;snapshot;pmh_insert;delete_non_keep;evidence_strength;rollup_step5',
            '0', '0', '0',
...
def main():
    ...
    print(f"mig_98d seroma apply — do_writes={do_writes}  ts={ts_tag}")
    con = _connect_md()
    try:
        before = _counts(con)
