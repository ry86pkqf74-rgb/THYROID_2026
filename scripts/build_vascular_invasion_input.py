#!/usr/bin/env python3
"""
Build vascular_invasion input parquet.

Prompt entity types: vascular_invasion, vascular_invasion_type (micro/macro/
angioinvasion), vessel_count, capsular_invasion, soft_tissue_invasion (ETE),
perineural_invasion_detailed, ki67_index, mitotic_rate, necrosis,
dedifferentiation, tall_cell_percentage, synoptic_report, ptnm_stage.

All of these live in path_synoptics (structured + narrative) with the richest
signal in microscopic_description / synoptic_diagnosis / tumor_X_histology_comment
and the angiolymphatic-specific margin_comment for tumor_1. Op notes add
intra-operative vessel-involvement signal.

Sources:
  1. path_synoptics whole-record narrative (8 cols):
       synoptic_diagnosis, microscopic_description, path_diagnosis_summary,
       path_diagnosis_comment, ancillary_studies, path_special_studies,
       other_findings, path_extended_gross_path
     note_date = surg_date
  2. path_synoptics per-tumor histology comments (tumors 1–3; 4–5 absent).
  3. path_synoptics per-tumor margin/angiolymphatic comments (tumors 1–5).
  4. path_synoptics per-tumor LN examined comment (tumor 1 only).
  5. clinical_notes_long op notes (opnote_1..4) — intra-op vascular
     invasion dictation. note_date via LEFT JOIN path_synoptics.

Writes: processed/remaining/9domain_v4/input_vascular_invasion.parquet
"""
...
PATH_TUMOR_LN_COMMENT: list[tuple[str, str]] = [
...
def build_union_sql() -> str:
    ...
    for col, ordinal in (
        PATH_TUMOR_HIST_COMMENT + PATH_TUMOR_MARGIN_COMMENT + PATH_TUMOR_LN_COMMENT
    ):
        legs.append(
            f"""
            SELECT research_id::VARCHAR     AS research_id,
