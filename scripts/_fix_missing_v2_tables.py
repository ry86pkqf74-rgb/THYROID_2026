...
WITH ps AS (
    SELECT
        ...
        TRY_CAST(tumor_1_size_greatest_dimension_cm AS DOUBLE) AS ps_size_cm_1,
        ...
        NULL::VARCHAR AS ps_tumor_count,
        NULL::BOOLEAN AS ps_multifocal,
        NULL::VARCHAR AS ps_consult_diagnosis
    FROM path_synoptics
...
tp AS (
    SELECT
        ...
        CAST(tumor_1_extrathyroidal_ext AS VARCHAR) AS tp_ete,
        tumor_1_gross_ete AS tp_gross_ete,
        tumor_1_size_cm AS tp_size_cm,
...
SELECT
    ...
    COALESCE(ps.ps_size_cm_1, TRY_CAST(tp.tp_size_cm AS DOUBLE)) AS tumor_size_cm,
...
WITH us_unpivot AS (
    SELECT
        ...
        GREATEST(
            TRY_CAST(nodule_1_height_mm AS DOUBLE),
            TRY_CAST(nodule_1_width_mm AS DOUBLE),
            TRY_CAST(nodule_1_length_mm AS DOUBLE)
        ) / 10.0 AS size_cm_max,
...
ct_nodules AS (
    SELECT
        ...
        NULL::VARCHAR AS exam_impression_raw,
        CASE WHEN pathologic_lymph_nodes IS TRUE OR lymph_nodes_suspicious IS TRUE
             THEN TRUE ELSE FALSE END AS suspicious_node_flag,
        CASE WHEN pathologic_lymph_nodes IS TRUE THEN 'yes'
             WHEN lymph_nodes_suspicious IS TRUE THEN 'yes'
             ELSE NULL END AS suspicious_node_details,
        FALSE AS growth_flag,
        FALSE AS dominant_nodule_flag
    FROM ct_imaging
    WHERE research_id IS NOT NULL
      AND (thyroid_nodule IS TRUE
           OR LOWER(COALESCE(CAST(thyroid_nodule AS VARCHAR),'')) LIKE '%yes%'
           OR LOWER(COALESCE(CAST(thyroid_nodule AS VARCHAR),'')) LIKE '%present%')
),
mri_nodules AS (
    SELECT
        CAST(research_id AS INTEGER) AS research_id,
        ...
        NULL::VARCHAR AS exam_impression_raw,
        CASE WHEN pathologic_lymph_nodes IS TRUE THEN TRUE ELSE FALSE END AS suspicious_node_flag,
        CASE WHEN pathologic_lymph_nodes IS TRUE THEN 'yes' ELSE NULL END AS suspicious_node_details,
        FALSE AS growth_flag,
        FALSE AS dominant_nodule_flag
    FROM mri_imaging
    WHERE research_id IS NOT NULL
...
WITH mt AS (
    SELECT
        ...
        CASE WHEN LOWER(COALESCE(mutation,'') || ' ' || COALESCE(detailed_findings,''))
             LIKE '%braf%' THEN TRUE ELSE FALSE END AS braf_flag,
        CASE WHEN LOWER(COALESCE(mutation,'') || ' ' || COALESCE(detailed_findings,''))
...
def main() -> None:
    ...
    for name, sql in [
        ...
        ("imaging_nodule_long_v2", IMAGING_SERIAL_SQL),
        ("molecular_test_episode_v2", MOLECULAR_SQL),
        ("rai_treatment_episode_v2", RAI_SQL),
        ("operative_episode_detail_v2", OPERATIVE_SQL),
        ("fna_episode_master_v2", FNA_SQL),
        ("imaging_exam_summary_v2", IMAGING_EXAM_SQL),
        ("event_date_audit_v2", EVENT_DATE_AUDIT_SQL),
        ("patient_cross_domain_timeline_v2", PATIENT_TIMELINE_SQL),
    ]:
        print(f"\nCreating {name}...")
        try:
            con.execute(sql)
