...
classified AS (
  SELECT *,
    CASE
      -- MALIGNANT: any carcinoma/cancer pattern
      WHEN regexp_matches(all_text,
        ...
        '|oncocytic.*carcinoma'
        '|metastatic.*(carcinoma|thyroid|ptc|mtc)'
        '|insular.*carcinoma'
      ...
      -- BORDERLINE: NIFTP, FTUMP, WDT-UMP, uncertain malignant potential
      WHEN regexp_matches(all_text,
        'niftp|ftump|wdt-ump'
      ...
      -- BENIGN: must match benign pattern AND NOT match malignant pattern
      WHEN regexp_matches(all_text,
        ...
        '|hashimoto|graves|lymphocytic thyroiditis'
        ...
        '|hurthle cell adenoma|oncocytic adenoma'
      ...
      -- Catch additional benign-only patterns
      WHEN regexp_matches(all_text, 'thyroiditis|goiter|hyperplasia|adenoma')
        AND NOT regexp_matches(all_text,
          'carcinoma|malign|metastatic|anaplastic|poorly differentiated|lymphoma|niftp|ftump|uncertain'
        ) THEN 'benign'

      -- Has text but unclassifiable by regex
