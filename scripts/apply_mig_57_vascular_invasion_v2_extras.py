#!/usr/bin/env python3
"""Idempotent follow-up to qc_framework_v1/migrations/57_vascular_invasion_v2_*.sql

- Renames main.note_entities_llm_vascular_invasion →
  main._deprecated_note_entities_llm_vascular_invasion when the former still exists
- Registers the deprecated table in detail_table_registry_v1 (replaces any row for
  note_entities_llm_vascular_invasion or the _deprecated_ name)
"""
...
def main() -> int:
    ...
    legacy = "note_entities_llm_vascular_invasion"
    depr = "_deprecated_note_entities_llm_vascular_invasion"
    ...
    con.execute(
        """
        COMMENT ON TABLE main._deprecated_note_entities_llm_vascular_invasion IS
        '[deprecated 2026-04-24] — superseded by main.note_entities_llm_vascular_invasion_v2
        and main.canonical_vascular_invasion_events_v1. Renamed from note_entities_llm_vascular_invasion.'
        """
    )
    con.execute(
        f"""
    ...
    con.execute(
        ...
        SELECT
          ...
          'legacy lvi-style columns; use note_entities_llm_vascular_invasion_v2 + canonical_vascular_invasion_*',
          '2026-04-24: Renamed from note_entities_llm_vascular_invasion; superseded by v2 + canonical_vascular_invasion.',
          'deprecated'
        FROM main."{depr}"
        """
    )
    print("[OK] detail_table_registry_v1: registered _deprecated_note_entities_llm_vascular_invasion")
    con.close()
    return 0
