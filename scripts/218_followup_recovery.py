...
def task1_followup_recovery(con: duckdb.DuckDBPyConnection, dry_run: bool) -> dict[str, Any]:
    ...
    concordance = con.execute("""
        ...
        JOIN _followup_patient_max_v2 f ON c.research_id = f.research_id
    ...
    # Recovery stats for patients with zero follow-up
    recovery = con.execute("""
        SELECT
            ...
            COUNT(*) AS n
        FROM canonical_patient_master_v1 c
        LEFT JOIN _followup_patient_max_v2 f ON c.research_id = f.research_id
    ...
    con.execute("""
        ...
        JOIN canonical_patient_master_v1 c ON f.research_id = c.research_id
...
def task4_recurrence_sites(con: duckdb.DuckDBPyConnection, dry_run: bool) -> dict[str, Any]:
    print("\n" + "=" * 70)
    ...
    rec_stats = con.execute(f"""
        SELECT
            COUNT(*) FILTER (WHERE recurrence_confirmed = TRUE) AS total_confirmed,
            COUNT(*) FILTER (WHERE recurrence_confirmed = TRUE AND recurrence_site IS NOT NULL)
                AS has_site,
            COUNT(*) FILTER (WHERE recurrence_confirmed = TRUE AND recurrence_site IS NULL)
                AS no_site
        FROM {CANONICAL}
    """).fetchone()
    ...
    con.execute("""
        ...
        WITH recurrence_fnas AS (
            ...
            FROM canonical_patient_master_v1 c
            JOIN fna_cytology f ON CAST(f.research_id AS VARCHAR) = c.research_id
            WHERE c.recurrence_confirmed = TRUE
        ...
        con.execute("""
            ...
            WHERE CAST(r.research_id AS VARCHAR) IN (
                ...
                WHERE recurrence_confirmed = TRUE
                  AND recurrence_site IS NULL
            )
            AND CAST(r.research_id AS VARCHAR) NOT IN (
                SELECT research_id FROM _recurrence_fna_sites_v1
            )
    ...
    con.execute(f"""
        UPDATE {CANONICAL} AS c
        SET
            recurrence_site = f.recurrence_site_standardized,
    ...
    if event_sites > 0:
        con.execute(f"""
            UPDATE {CANONICAL} AS c
            SET
                recurrence_site = e.recurrence_site_standardized,
            ...
            WHERE c.research_id = e.research_id
              AND c.recurrence_site IS NULL
        """)

    # Post check
    post = con.execute(f"""
        SELECT COUNT(*) FILTER (WHERE recurrence_site IS NOT NULL)
        FROM {CANONICAL} WHERE recurrence_confirmed = TRUE
    """).fetchone()[0]
    print(f"  Post-update: {post} of {rec_stats[0]} confirmed recurrences have site")
...
def task6_canonical_rebuild(con: duckdb.DuckDBPyConnection, dry_run: bool) -> dict[str, Any]:
    print("\n" + "=" * 70)
    ...
    if table_exists(con, "_followup_computed_v2"):
                ...
                last_contact_date = CASE
                    WHEN f.followup_years_v2 >= 0
                      AND (c.last_contact_date IS NULL
                           OR f.last_contact_date_v2 > c.last_contact_date)
        ...
        fu_stats = con.execute(f"""
            SELECT
                COUNT(*) FILTER (WHERE followup_years > 0) AS positive_fu,
                COUNT(*) FILTER (WHERE followup_years = 0 OR followup_years IS NULL) AS zero_fu,
                ROUND(PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY followup_years)
                    FILTER (WHERE followup_years > 0), 2) AS median_fu
            FROM {CANONICAL}
        """).fetchone()
    ...
    if table_exists(con, "_nucmed_labs_rollup_v1"):
        con.execute(f"""
            UPDATE {CANONICAL} AS c
            SET
                rai_stimulated_tsh = COALESCE(c.rai_stimulated_tsh, nm.rai_stimulated_tsh_v2),
                rai_stimulated_tg = COALESCE(c.rai_stimulated_tg, nm.nucmed_tg_max),
            ...
            WHERE c.research_id = nm.research_id
        """)

        stim_tsh = con.execute(f"""
            SELECT COUNT(*) FILTER (WHERE rai_stimulated_tsh IS NOT NULL) FROM {CANONICAL}
        """).fetchone()[0]
        stim_tg = con.execute(f"""
            SELECT COUNT(*) FILTER (WHERE rai_stimulated_tg IS NOT NULL) FROM {CANONICAL}
        """).fetchone()[0]
    ...
    total_rec = con.execute(f"""
        SELECT COUNT(*) FROM {CANONICAL} WHERE recurrence_confirmed = TRUE
    """).fetchone()[0]
    ...
    prov = con.execute(f"""
        SELECT
            'followup' AS domain,
            COUNT(*) FILTER (WHERE last_contact_source IS NOT NULL) AS has_source,
            COUNT(*) FILTER (WHERE last_contact_date IS NOT NULL) AS has_date,
        ...
        SELECT 'nucmed_tsh',
            COUNT(*) FILTER (WHERE nucmed_lab_source IS NOT NULL
                AND rai_stimulated_tsh IS NOT NULL),
        ...
        SELECT 'recurrence_site',
            COUNT(*) FILTER (WHERE recurrence_site_source IS NOT NULL
                AND recurrence_site IS NOT NULL),
...
def main() -> None:
    ...
    if run_all or args.phase == "4":
        all_results["task4"] = task4_recurrence_sites(con, args.dry_run)

    if run_all or args.phase == "5":
        all_results["task5"] = task5_complication_crossval(con, args.dry_run)

    if run_all or args.phase == "6":
        all_results["task6"] = task6_canonical_rebuild(con, args.dry_run)
