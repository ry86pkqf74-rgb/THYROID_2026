...
def main() -> None:
    ...
    try:
        ...
        for name, stem, tier, qa_tier, pq_path in domains_to_load:
            ...
            md_count = con.execute(f"SELECT COUNT(*) FROM v2_stage.{stem}").fetchone()[0]
            match = local_count == md_count

            con.execute(
                "INSERT INTO v2_stage.load_inventory VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                [next_id, name, stem, tier, qa_tier, str(pq_path), local_count, md_count,
                 match, now, sha, reg_ver],
            )
            next_id += 1

            status = "OK" if match else "MISMATCH"
            print(f"  [{status}] v2_stage.{stem}: local={local_count:,}  md={md_count:,}")
            if not match:
                mismatches.append(stem)
            loaded += 1

        print(f"\n  [summary] {loaded} tables loaded into v2_stage")
        print(f"  [summary] {len(mismatches)} row-count mismatches")

        if mismatches:
            print(f"  [error] Mismatches in: {mismatches}")
            sys.exit(1)

        inv_count = con.execute("SELECT COUNT(*) FROM v2_stage.load_inventory").fetchone()[0]
