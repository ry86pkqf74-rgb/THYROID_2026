-- Reconciliation V2 Views
-- Generated by 16_reconciliation_v2.py
-- Deploy to thyroid_research_2026 via: USE thyroid_research_2026;
--
-- Phases: A (histology), B (molecular), C (RAI),
--         D (timeline rescue), E (validation), F (analytics)
--
-- Depends on: 15_date_association_views.sql (enriched_* views)
    ...
    SELECT
        ...
        CASE
            ...
            WHEN regexp_matches(CAST(mt."date" AS VARCHAR), '^\d{4}$')
                THEN TRY_CAST(CAST(mt."date" AS VARCHAR) || '-07-01' AS DATE)
            ELSE NULL
        END AS molecular_date,
        CASE
            WHEN TRY_CAST(mt."date" AS DATE) IS NOT NULL THEN 'day'
            WHEN regexp_matches(CAST(mt."date" AS VARCHAR), '^\d{4}$')
                THEN 'year'
            ELSE 'invalid'
        END AS molecular_date_granularity,
        CAST(mt."date" AS VARCHAR) AS specimen_date_raw,
        ...
        ROW_NUMBER() OVER (
            ...
            ORDER BY
                CASE WHEN TRY_CAST(mt."date" AS DATE) IS NOT NULL THEN 0 ELSE 1 END,
                TRY_CAST(mt."date" AS DATE) ASC NULLS LAST,
                mt.test_index ASC
