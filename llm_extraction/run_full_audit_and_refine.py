...
Usage:
    ...
    python -m llm_extraction.run_full_audit_and_refine --all --md

    # Single entity audit
    python -m llm_extraction.run_full_audit_and_refine --entity hypocalcemia --md

    # Inventory only (print ranked entity list, no execution)
    python -m llm_extraction.run_full_audit_and_refine --inventory-only

    # Deploy refinement only (skip audit, just re-deploy SQL)
    python -m llm_extraction.run_full_audit_and_refine --refine-only --md

    # Dry run (show what would happen)
    python -m llm_extraction.run_full_audit_and_refine --all --md --dry-run
"""
...
def run_audit_all(con, sample_size: int = 200, verbose: bool = True) -> dict:
    ...
    from llm_extraction.extraction_audit_engine import master_audit, build_summary_table
...
def run_audit_entity(con, entity: str, sample_size: int = 200, verbose: bool = True) -> dict:
    ...
    from llm_extraction.extraction_audit_engine import audit_entity
...
def run_refinement(con, dry_run: bool = False, verbose: bool = True) -> dict:
    ...
    from llm_extraction.complications_refined_pipeline import run_pipeline
