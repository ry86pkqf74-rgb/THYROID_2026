...
This study uses a publication-grade BigQuery canonical (10,871 patients; goiter subset 6,075) to characterize race-stratified MNG presentation, preoperative workup, and perioperative outcomes. We report time from first preoperative FNA to surgery as a tempo metric not commonly included in disparities analyses but well-encoded in our canonical (`prm_first_fna_days_from_surg`, 48.1% non-null in the goiter cohort). We adopt the institution-wide complication-temporality reporting rule (transient/permanent hypoparathyroidism; preoperative yes/no for hypocalcemia where encoded; postoperative-confirmed only with footnote for RLN and vocal-cord paralysis pending preop encoding migrations `MIG-001` and `MIG-002`). We are explicit that race is the only structured SDOH variable available and that further inference will require linkage to ZIP- or census-block-group-level deprivation data.
...
6. Articulate the inferential limits imposed by the absence of structured SDOH variables beyond race, and identify carry-forward migrations needed for full preoperative complication reporting.
...
- **RLN injury, VC paralysis, VC paresis:** Postoperative confirmed only. Preoperative status not currently encoded as a structured field on `canonical_patient_master` (carry-forwards `MIG-001` `CF-RLN-PREOP-FLAG`, `MIG-002` `CF-VC-PARALYSIS-PREOP-FLAG` open). Footnote applies to all three rows of Table 3.
...
4. **Recurrent-laryngeal-nerve and vocal-cord preoperative status not encoded.** Carry-forwards `MIG-001` (Linear THY-13) and `MIG-002` (Linear THY-14) are open. Postoperative-confirmed counts therefore cannot be subtracted by preexisting cases. Until these encodings land, the postoperative-confirmed-only rows in Table 4 are conservative — true incidence of new postoperative injury is at most the rate reported.
...
10. **No source-stratified VC findings column.** Imaging (`mri_vocal_cords_*`), laryngoscopy (`proc_nlp_laryngoscopy`, `ops_preop_laryngoscopy`), operative (`op_nlp_rln_finding`, `syn_io_rln_monitoring`), and NSQIP (`nsqip_rln_injury_flag`) signals exist independently. Carry-forward `MIG-004` (Linear THY-16) will add `vc_finding_source_first` and `vc_finding_source_set` derivations.
