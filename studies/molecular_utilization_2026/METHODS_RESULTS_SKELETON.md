...
**V2 implementation.** `sql/01_views_and_cohort_v2.sql` materializes **index FNA–based** eligibility (first Bethesda III/IV FNA on/after 2015, intersected with `manuscript_cohort_v1` III/IV), patient-level molecular uptake (ThyroSeq/Afirma on/after index), nested **operated** and **Bethesda V** shells, **episode_sensitivity_v2** (patient vs episode vs FNA–molecular v3 overlap), structured pathology with `histology_final` fallback, and exports under `outputs/v2/` via `python/run_analysis.py --v2`.
