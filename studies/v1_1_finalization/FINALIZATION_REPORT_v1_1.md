...
## v1_1 Closeout Addendum (2026-04-17)

Per Prompt 13 closeout (Scripts 260-264). This section is APPENDED, not a rewrite.

**Run timestamp (UTC):** 2026-04-17T02:55:41.115540+00:00  
**Branch:** `cleanup/v1_1_finalization-20260416`

### Executed in this round

| Script | Subject | Status |
|---|---|---|
| 260 | Hydrate `fna_episode_master_v2` linkages | deferred (decision_pending; see scripts/output/260_decision_pending.md) |
| 261 | Repair `fna_history` two-digit-year dates | applied (24 dates repaired, 2 nulled) |
| 262 | Drop `ras_positive_v7` + cast FEM.research_id to VARCHAR | applied (CPM cols 1494->1493; FEM.research_id INTEGER->VARCHAR) |
| 263 | Resolve `bethesda_final` semantic ambiguity | deferred (decision_pending; see scripts/output/263_semantic_decision_pending.md) |

### Invariants (this round)

| # | Invariant | Result | Target | Status |
|---|---|---|---|---|
| 1 | CPM row count | 10,871 | 10,871 | PASS |
| 2 | CPM column count | 1,493 | 1,493 | PASS |
| 3 | `ras_positive_v7` absent on CPM | absent | absent | PASS |
| 4 | `fna_episode_master_v2.research_id` dtype | VARCHAR | VARCHAR | PASS |
| 5 | `fna_episode_master_v2` row count | 8,119 | 8,119 | PASS |
| 6 | FEM bad `resolved_fna_date` rows | 0 | 0 | PASS |
| 7 | FEM B III/IV linked_molecular_episode_id pct | 0.0% (0/1920) | >= 85% | pending_decision |
| 8 | `__conventions.bethesda_semantics` row | pending_decision | present | pending_decision |

### Script 259 replay queries (must remain 0)
...
### CPM hash-of-row-hashes (candidate v1_1 lock hash)

`b8631b86f6ddcbadc0b34f320e8ec52b28bb055d7991bfd86e07ba61ef143fbd`

Computed as `sha256(concat(research_id || md5(cpm row) || newline) ordered by research_id)`. Re-run Script 264 to recompute and compare.

### Open decisions (deferred to Logan)

- **Script 260** - molecular FK linkage: Strict-90d coverage 29.69% < 85% floor (MTE date sparsity); see scripts/output/260_decision_pending.md for Path A/B/C.
- **Script 263** - `bethesda_final` semantics: see `scripts/output/263_semantic_decision_pending.md`.

---

_End of v1_1 Closeout Addendum (Script 264)._
