...
children.push(new Paragraph({
  ...
  spacing: { after: 120 },
  children: [
    new TextRun({ text: "Working draft v2 — EXT2-4 Elicit expansion (2026-05-09)", italics: true }),
  ],
}));
...
children.push(pmix([
  bold("Results. "),
  new TextRun(
    "8,368 surgical patients met inclusion (78.5% female; median age 52, IQR 40–63). Preoperative imaging-defined 2.0–4.0 cm subgroup: n=400 (392 in 2015+ era; 222/400 (55.5%) initial total thyroidectomy). Bethesda III/IV with named platform and final histology: n=180 evaluable (Afirma 76; ThyroSeq 104, plus 15 ThyroSeq INTERMEDIATE-band as a separate category). Overall Bethesda III/IV diagnostic performance (strict, NIFTP=benign): Afirma sensitivity 89.4% (95% CI 77.4–95.4), specificity 17.2% (7.6–34.5), PPV 63.6% (51.6–74.2), NPV 50.0% (23.7–76.3); ThyroSeq sensitivity 67.3% (53.4–78.8), specificity 61.8% (48.6–73.5), PPV 61.1% (47.8–73.0), NPV 68.0% (54.2–79.2). In the Bethesda III/IV 2–4 cm subgroup (n=19 evaluable for ThyroSeq; n=4 for Afirma), ThyroSeq performed 88.9/90.0/88.9/90.0 across the four metrics; the Afirma cell was too small for stable estimation. ThyroSeq numeric ROM% tracked the descriptive bands (median ROM% 3% for negative call, 50% for INTERMEDIATE, 70% for positive call). Path-proven recurrence among malignant cases was 0/137 in Afirma (follow-up artifact, all post-2015), 4/161 (2.5%) in ThyroSeq, and 68/2,538 (2.7%) in patients with non-named-platform molecular evidence."
  ),
]));
...
children.push(pmix([
  bold("Conclusions. "),
  new TextRun(
    "When evaluated by the actual platform-reported call rather than a derived risk tier, Afirma's GSC behaves like a high-sensitivity rule-out test (89% sensitivity, 17% specificity) and ThyroSeq's binary band call shows balanced performance (67%/62%) with a separately reported INTERMEDIATE category. In the 2–4 cm Bethesda III/IV subgroup specifically, ThyroSeq metrics align tightly at 89–90% across sensitivity, specificity, PPV, and NPV (n=19); the corresponding Afirma cell is too small for stable estimation. All conditional-on-surgery operating characteristics in this dataset are subject to verification bias because molecular-negative patients largely avoid surgery and are unobservable; specificity and NPV are most affected. A non-trivial 165 ThyroSeq tests in this cohort have a non-classifiable reported call due to incomplete band extraction—a parser-completeness gap that bounds the precision of the ThyroSeq estimate."
  ),
]));
...
children.push(p(
  "Among 8,368 surgical patients, 933 had Bethesda III/IV cytology and 497 had a named molecular platform; 180 patients had a classifiable platform-reported call AND a final histology. After patient-level deduplication (latest preoperative test) the 2×2-evaluable subset comprised 76 Afirma (B3 34, B4 42) and 104 ThyroSeq (B3 72, B4 32) patients, plus 15 ThyroSeq INTERMEDIATE-band patients reported separately."
));
children.push(p(
  "In Bethesda III/IV all sizes (Strict, NIFTP=benign): Afirma sensitivity was 89.4% (95% CI 77.4–95.4), specificity 17.2% (7.6–34.5), PPV 63.6% (51.6–74.2), NPV 50.0% (23.7–76.3); ThyroSeq sensitivity was 67.3% (53.4–78.8), specificity 61.8% (48.6–73.5), PPV 61.1% (47.8–73.0), NPV 68.0% (54.2–79.2). In the Bethesda III/IV 2–4 cm subgroup ThyroSeq sensitivity, specificity, PPV, and NPV all clustered at 88.9–90.0% (n=19 evaluable); the corresponding Afirma cell (n=4) was too small to interpret. The ThyroSeq INTERMEDIATE band (n=15 across B3+B4) was 47% malignant on final histology, sitting between the negative band (32% malignant on path) and the positive band (65% malignant on path) as expected. Full Strict and Inclusive (NIFTP=malignant) tables, including B3-only and B4-only subgroup rows, appear in Table 3 v2 and Figure 2."
));
children.push(...imageFig("fig2_forest_diagnostic_performance.png",
  "Figure 2. Forest plot of sensitivity, specificity, PPV, and NPV (Wilson 95% CIs) for Afirma vs ThyroSeq in Bethesda III/IV; Strict NIFTP=benign rule. The 2–4 cm Afirma cell (n=4) is shown for completeness but is too small for stable interpretation."));

children.push(h(HeadingLevel.HEADING_2, "ThyroSeq numeric ROM% by reported call and histology (Figure 3)"));
children.push(p(
  "ThyroSeq numeric ROM% (rom_percent_point) tracked the descriptive bands without ambiguity. Negative-call median ROM was 3% [IQR 3–3] for benign histology (n with numeric ROM=29) and 3% [3–3] for malignant histology (n=14). INTERMEDIATE-call median ROM was 50% [40–50] for benign (n=8) and 50% [40–70] for malignant (n=7). Positive-call median ROM was 70% [60–70] for benign (n=17) and 70% [70–70] for malignant (n=31). The numeric ROM% in this cohort therefore validates the band assignments without contradiction (Figure 3)."
));
...
children.push(p(
  "Two methodological observations are worth flagging for follow-up. First, 165 ThyroSeq tests in this cohort have a non-classifiable reported call (rom_descriptor IS NULL AND overall_result_class NOT IN ('positive','negative')) despite the underlying clinical reports presumably containing a band assignment. Roughly half of those (49% of 165) went to malignant histology, suggesting these are not random missingness but include a meaningful share of clinically called tests where our extraction did not capture the band. A parser-completeness verification check is recommended (filed as VC-MOL-PARSE-001 in the institutional Verification Checks table for the next session), and the ThyroSeq performance estimates here are conservative until that gap is resolved. Second, the 2–4 cm subgroup analysis is underpowered for Afirma in this institutional cohort; multi-center pooling with reported-call—rather than risk-tier—granularity would be valuable."
));
...
children.push(table([
  ...
  ["Afirma B3+B4 all sizes", "42", "24", "5", "5", "76", "89.4 [77.4–95.4]", "17.2 [7.6–34.5]", "63.6 [51.6–74.2]", "50.0 [23.7–76.3]"],
  ["Afirma B3+B4 2–4 cm", "3", "0", "1", "0", "4", "75.0 [30.1–95.4]", "—", "100.0 [43.8–100.0]", "0.0 [0.0–79.3]"],
  ["ThyroSeq B3+B4 all sizes", "33", "21", "16", "34", "104", "67.3 [53.4–78.8]", "61.8 [48.6–73.5]", "61.1 [47.8–73.0]", "68.0 [54.2–79.2]"],
  ["ThyroSeq B3+B4 2–4 cm", "8", "1", "1", "9", "19", "88.9 [56.5–98.0]", "90.0 [59.6–98.2]", "88.9 [56.5–98.0]", "90.0 [59.6–98.2]"],
