...
- (f) **Date-completeness provenance skew (added 2026-05-13 post-mig_324, commit 3068a81).** `canonical_molecular_genetics_v2.resolved_test_date` now has 100% coverage (was 35% pre-fix), but only **35% (481/1,384) are native test dates**; the remaining **65% (903/1,384) are `imported_at` fallback dates** (file upload to `thyroseq_molecular_enrichment`). The intended FNA-linkage backfill arm yielded zero rows because of a structural key-type mismatch between `canonical_molecular_genetics_v2.linked_fna_episode_id` (numeric episode tokens) and `canonical_fna_events_v1.fna_event_id` (32-character hex). Tracked as `VC-MOL-DATE-BRIDGE-001` and `NF-2026-05-13-canonical-molecular-date-coverage-with-fna-bridging-gap`. **Era stratification at the year level (pre-2015 vs 2015+) is reliable; sub-year temporal claims — e.g., FNA-to-molecular-test interval — should not be made from this dataset until the FNA-bridge is built.**
