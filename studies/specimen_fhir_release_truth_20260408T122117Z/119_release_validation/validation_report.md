...
**Generated:** 2026-04-08T12:21:30.694485+00:00
**Mode:** Release Validation
**Total checks:** 25
**Passed:** 19  |  **Warned:** 0  |  **Failed:** 6

**VERDICT: BLOCKED** — 6 check(s) failed.
...
| Check | Status | Detail |
|-------|--------|--------|
| MD attachment | PASS | 12 databases attached |
| Row count parity | PASS | 23 domains checked, all match |
| Canonical canonical_extracted_fact_long_v2 | FAIL | local=-1  md=123,577 |
| Canonical canonical_fact_quarantine_v2 | FAIL | local=-1  md=199 |
| Canonical note_extraction_runs | FAIL | local=-1  md=3 |
...
| Review queue (synthetic placeholder) | FAIL | 5,620 row(s) have publication-blocked synthetic placeholder verification_status (canonical: 'SYNTHETIC_AUTOMATION_ONLY_NOT_MANUSCRIPT_SIGNOFF'; see docs/publication_governance_gate.md). Replace via human-reviewed CSV or scripts/128_mrq_tier_policy_gate_build.py. For rehearsal only, run 119 without --release-mode. |
| Promotion decision provenance | FAIL | 2 / 2 rows have NULL/blank decision_batch_id — publication releases must record batch provenance (126 --decision-batch-id / append path). |
...
| Release manifest | PASS | 6 release(s); latest: 20260407_final2 (2026-04-07 05:11:41.171561) |
...
| Molecular layer present | PASS | main.molecular_results not found — molecular contract checks skipped |
| Molecular episode upstream spine | FAIL | main.molecular_testing is missing but molecular_test_episode_v2 has 10,126 rows — catalog lacks script-22 source spine; test_date_native is mostly NULL, so fna_molecular_linkage_v3 cannot attach genomics to FNA/specimen (load molecular_testing, rerun scripts/22_canonical_episodes_v2.py --md for molecular episodes, then scripts/49_enhanced_linkage_v3.py --md and scripts/140_md_specimen_genomics_binding.py --md). |
| Specimen layer prerequisites | PASS | main.synoptic_tumor_long_v1 absent — specimen/FHIR checks skipped (run 108 + 109 + 138) |
