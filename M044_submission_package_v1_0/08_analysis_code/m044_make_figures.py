...
Figures:
  1. Cohort flow diagram (N → strict-DTC → analytic primary → Cox/KM aligned N).
  ...
  6. Kaplan-Meier — same sample and time axis as Cox PH (`time_days`/365.25 years).
...
def fig1_cohort_flow(df: pd.DataFrame):
    """Figure 1 — cohort flow; Cox/KM box matches lifelines analytic frame."""
    hs = df["histology_final"].map(_histology_str_raw)
    excl_strict = hs.isin(EXCLUDE_HISTOLOGIES_STRICT_DTC)

    n_total = len(df)
    excluded_n = int(excl_strict.sum())
    strict = df.loc[~excl_strict]
    n_strict = len(strict)
    primary = strict[strict["ete_group"].isin(ORDER)]
    n_prim = len(primary)
    excl_other_ete = n_strict - n_prim

    dm_strict = model_frame_for_primary(df, strict_dtc=True)
    cox_df, _cox_meta = build_cox_analytic_frame(dm_strict)
    n_cox = len(cox_df)
    ...
    boxes = [
        ...
        (
            ...
            6.5,
            4.0,
            f"Cox PH + KM subset (aligned sample)\nn = {n_cox}",
            "#F9E79F",
        ),
    ]
    ...
    ax.text(
        7.0,
        4.75,
        f"→ Same filters as Cox: surg date, FU>0,\n   finite Cox duration + age/size\n"
        f"   (excluded {max(0, n_prim - n_cox)})",
        fontsize=9,
        color='#7F8C8D',
    )
    ...
    pd.DataFrame([
        ...
        ("Cox PH / KM analytic (aligned)", n_cox),
...
def fig2_ete_distribution(df: pd.DataFrame):
    ...
    hs = df["histology_final"].map(_histology_str_raw)
    strict = df.loc[~hs.isin(EXCLUDE_HISTOLOGIES_STRICT_DTC), "ete_group"].value_counts().reindex(ORDER + ["Present ungraded", "Missing/other"]).fillna(0).astype(int)
...
def fig6_km(df: pd.DataFrame):
    """Figure 6 — KM path-proven RFS; same rows/durations as Cox PH (time_days / 365.25)."""
    dm_strict = model_frame_for_primary(df, strict_dtc=True)
    cox_df, _ = build_cox_analytic_frame(dm_strict)
    cox_df = cox_df.loc[cox_df["ete_group"].isin(ORDER)].copy()
    cox_df["T_years"] = pd.to_numeric(cox_df["time_days"], errors="coerce") / 365.25

    fig, ax = plt.subplots(figsize=(8, 6))
    rows_export: list[dict] = []
    tmax = float(cox_df["T_years"].max()) if len(cox_df) else 15.0

    if len(cox_df) == 0:
        ax.text(
            0.5,
            0.5,
            "No Cox-aligned rows — check parquet + filters",
            ha="center",
            va="center",
        ...
        for g in ORDER:
            d = cox_df[cox_df["ete_group"].astype(str) == g]
            if len(d) == 0:
                continue
            kmf = KaplanMeierFitter()
            T = d["T_years"].to_numpy(dtype=float)
            E = d["y_pp"].astype(int).to_numpy()
                    ...
                    rows_export.append(
                        {
                            ...
                            "year": tp,
                            "rfs_pct": surv * 100,
                            "n_at_risk": int((T >= tp).sum()),
                        }
                    )
                except Exception:
                    pass
    ax.set_xlabel("Years on Cox time axis (path-proven TTE; censored at follow-up)")
    ax.set_ylabel("Path-proven recurrence-free survival")
    ax.set_title(
        "Figure 6. KM path-proven RFS by ETE (strict-DTC; aligned with Cox PH sample)"
    )
    ax.set_ylim(0.5, 1.02)
    ax.set_xlim(0, min(15.0, tmax if np.isfinite(tmax) and tmax > 0 else 15.0))
    ax.grid(alpha=0.3)
    plt.tight_layout()
