...
- DuckDB/MotherDuck UPDATE rowcount returns -1 (safe_exec converts to 0); use Phase G delta validation tables to verify actual changes, not in-flight rowcount- Script 105 (`105_manuscript_freeze_v1.py`): 7-phase manuscript publication freeze — environment verify, table inventory, Parquet/CSV export, checksum, manifest, optional MotherDuck stamp, integrity verify; supports `--md`, `--dry-run`, `--stamp`, `--version TAG`, `--skip-data`
