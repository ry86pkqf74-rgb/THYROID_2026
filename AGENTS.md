...
- When saving outputs, ensure changes are reflected in both DuckDB online (local DuckDB) and GitHub where applicable; analysis artifacts go to GitHub; schema/data changes go to local DuckDB
...
- Require explicit confirmation before modifying local DuckDB tables; no production changes without user consent
...
- Apply defense-in-depth fix strategy: fix both the Python consumer (e.g. `is True`, `or 0`) AND the SQL source (e.g. `COALESCE(..., FALSE)`) in a single task; deploy SQL fix to local DuckDB immediately after code changes
...
- Validation/cross-check scripts must explicitly compare live local DuckDB extraction vs. saved CSVs row-by-row and flag any discrepancy >0.1% in a dedicated report section
- For single-view local DuckDB fixes, prefer targeted SQL execution via `.venv/bin/python -c "import duckdb; ..."` pattern rather than re-running entire script chains (e.g. rebuild just `streamlit_patient_header_v` instead of full script 03 or 18→26)
...
- Data access: use `local DuckDB_client.py` or local `thyroid_master.duckdb`; fallback to CSV exports when DuckDB unavailable
- `LOCAL_DB_PATH` can be loaded from `.streamlit/secrets.toml` when not set in env
- `thyroid_share` (RO share) is read-only; all writes (ALTER, UPDATE, CREATE VIEW) must target `thyroid_master.duckdb`
- `profile_note_entities.py` attaches the RW share as `thyroid_rw` (not `thyroid_share`) to avoid confusion with the RO convention
- DuckDB/local DuckDB does not support `CREATE OR REPLACE MATERIALIZED VIEW`; use `CREATE OR REPLACE TABLE` instead
...
- Upload note_entities to local DuckDB via `scripts/09b_local DuckDB_upload_notes_entities.py --confirm`
...
- local DuckDB requires DuckDB ≤1.4.4; v1.5.0 is incompatible
...
- `.venv/bin/python` is required for DuckDB/local DuckDB scripts; system Python lacks duckdb module
- `local DuckDB_client.py` supports `USE_LOCAL_DUCKDB` env var and `local DuckDBConfig.use_local` flag; `LOCAL_DUCKDB_PATH` env var defaults to `thyroid_master_local.duckdb`
...
- Script 28 now supports `--md` flag for local DuckDB reads
...
- Script 27 deployed to both local DuckDB and local DuckDB cloud; all 5 legacy compatibility tables verified (molecular_episode_v3: 11,977 rows, rai_episode_v3: 938, validation_failures_v3: 2,505, tumor_episode_master_v2: 13,202, linkage_summary_v2: 11,977)
...
- `03_research_views.py` local execution note: local `thyroid_master.duckdb` has a slim `tumor_pathology` (15 cols); full schema (ETE, invasion, mutation, LN columns) exists only in local DuckDB `thyroid_master.duckdb`. Run script with RW local DuckDB token, not against local DB
...
- local DuckDB stores boolean columns (`braf_positive`, `braf_mutation_mentioned`, etc.) as text `'true'`/`'false'`; use `LOWER(CAST(col AS VARCHAR)) = 'true'` for safe comparison
...
- Local `thyroid_master.duckdb` has empty `master_cohort`/`tumor_pathology` (0 rows); survival analysis must run with `--md` flag against local DuckDB
...
- Materialize as tables (not views) for RO share visibility; new views on RW (`thyroid_master.duckdb`) may not appear in the RO share
- `LOCAL_DB_PATH` in `.streamlit/secrets.toml` can be loaded via `toml.load('.streamlit/secrets.toml')['LOCAL_DB_PATH']` for scripts
...
- local DuckDB audit report at `studies/local DuckDB_audit/AUDIT_REPORT.md`
...
- ThyroSeq tables deployed to both local DuckDB and local DuckDB cloud; 8 tables total: `stg_thyroseq_excel_raw`, `stg_thyroseq_match_results`, `stg_thyroseq_parsed`, `thyroseq_molecular_enrichment`, `thyroseq_followup_labs`, `thyroseq_followup_events`, `thyroseq_fill_actions`, `thyroseq_review_queue`
...
- ThyroSeq race conflicts: local DuckDB `path_synoptics.race` uses "Black or African American" vs ThyroSeq "African American"; all 46 discordances routed to review, not auto-overwritten
...
- `patient_level_summary_mv` is the correct patient spine for views needing `histology_1_type` and `overall_stage_ajcc8`; `master_cohort` lacks these columns on local DuckDB
...
- `duckdb` CLI binary not installed locally; deploy SQL files via `.venv/bin/python -c "import duckdb; con = duckdb.connect('thyroid_master.duckdb'); con.execute(open('file.sql').read())"` pattern
...
- `recurrence_risk_features_mv` has 4,976 rows on local DuckDB; key columns: recurrence_flag (boolean stored as text), first_recurrence_date, recurrence_risk_band, braf_positive, ras_positive, tg_first/last/max/mean/measurement_count/annual_log_slope, ln_positive, ln_examined, ln_ratio
...
- `streamlit_patient_header_v` boolean columns (`histology_analysis_eligible`, `has_eligible_molecular`, `has_eligible_rai`) now use `COALESCE(..., FALSE)` in both script 18 (VIEW) and script 03 (TABLE); rebuilt on local DuckDB with 11,977 rows, 0 NULL booleans
- `patient_reconciliation_summary_v` on local DuckDB uses source column names: `has_high_risk_molecular` (not `has_eligible_molecular`), `has_definite_rai` (not `has_eligible_rai`), `total_issues` (not `total_review_items`/`review_priority_tier`); script 03 computes the renamed/derived columns inline; script 18 VIEW definition assumes pre-computed names from the view itself — use script 03 SQL pattern when rebuilding `streamlit_patient_header_v` directly on local DuckDB
...
- `thyroglobulin_labs` table exists only on local DuckDB (30,245 rows, 2,569 distinct patients); NOT in local `thyroid_master.duckdb`; columns: research_id, specimen_collect_dt (VARCHAR timestamp), result (includes `<0.2`-style thresholds as valid), units, lab_index; use `result IS NOT NULL AND TRIM(result) <> ''` for validity filter
...
- Streamlit Cloud surfaces `pd.NA` / `NAType.__bool__` crashes that local development hides — Cloud uses exact deployed pandas + local DuckDB driver versions; always test dashboard code with NULL-heavy patient records before deploying
- local DuckDB persisted tables are PHI-stripped (no MRN/DOB/name columns); patient matching to external data sources can only be done via raw Excel files in `raw/` that retain both PHI identifiers and `research_id`; `patient_level_summary_mv` has no MRN/DOB/name — demographic fallback (age+sex+surgery_date) is the only local DuckDB-native matching path
...
- Script 10 has no `--table` filter to deploy individual materialized tables; running it re-creates all tables in `MATERIALIZED_TABLES`; for deploying a single new table, use targeted Python one-liner against local DuckDB instead
...
- local DuckDB `information_schema.tables` returns each table multiple times (once per attached schema/database); use `SELECT DISTINCT table_name` or filter by `table_catalog` to avoid duplicate counts
...
- `imaging_nodule_long_v2` on local DuckDB has 10,866 rows but ALL size/TIRADS columns are NULL (schema exists, data not populated); preop imaging concordance is path_only for all 4,030 patients
...
- All downstream master tables (v10, v11, v12) rebuilt with corrected flags on local DuckDB
- Zenodo archive rebuilt at `exports/zenodo_archive_2026.03.10/` (229 files, 38.5 MB) after molecular fix
- local DuckDB materialization (script 26) re-run: all 131 md_ tables synced with corrected data; materialized copies verified MATCH source tables
- Provenance Traceability phase (script 46): `scripts/46_provenance_audit.py` — one-command local DuckDB provenance audit; run `--md` to audit all 334 tables, create `provenance_enriched_events_v1` (50,297 rows) + `lineage_audit_v1` (10,871 rows), and generate `docs/provenance_coverage_report.md` + `docs/date_accuracy_verification_report_YYYYMMDD.md`; run before `scripts/29_validation_engine.py --md` so upstream tables exist
...
- `scripts/46_provenance_audit.py`: single `--md` command; audits all local DuckDB tables for provenance coverage, runs lab-date accuracy SQL, creates `provenance_enriched_events_v1` (50,297 rows) and `lineage_audit_v1` (10,871 rows), inserts QA issues, generates markdown reports
...
- `information_schema.columns` on local DuckDB: always filter with `WHERE table_schema = 'main'` AND `DISTINCT column_name` — table returns duplicate rows per column otherwise
...
- Script 26 `--md` mode requires `LOCAL_DB_PATH` env var; export via `LOCAL_DB_PATH=$(.venv/bin/python -c "import toml; print(toml.load('.streamlit/secrets.toml')['LOCAL_DB_PATH'])")` prefix
- Zenodo/Streamlit sync: Zenodo archive rebuild = `scripts/32_zenodo_archive_prep.py`; local DuckDB sync = `scripts/26_local DuckDB_materialize_v2.py --md` with token; Streamlit auto-reads local DuckDB — no additional deployment step needed after materialization
- `stg_dob_excel_recovery` table (11,610 rows): cross-file DOB resolution from 6 sources (All Diagnoses Excel, Notes Excel, OP Sheet Excel + thyroid_weights, thyroglobulin_labs, anti_tg_labs); majority vote resolves 5,738 conflicts; priority tiebreak: all_diagnoses > op_sheet > thyroid_weights > notes > labs; `dob_concordant` boolean + `dob_resolution` provenance
- Raw Excel DOB sources: `All Diagnoses & synoptic 12_1_2025.xlsx` (DOB+Race+Gender+Age for 10,871 patients), `Notes 12_1_25.xlsx` (DOB for 8,743), `Thyroid OP Sheet data.xlsx` (Date of birth for 9,368); `Imaging_12_1_25.xlsx` has `Dominant nodule size on US` column but it is entirely empty (0 values)
- DOB recovery chain: 802 missing → 585 from thyroid_weights DB → 131 from OP Sheet Excel → 88 truly missing in all sources (mostly research_ids > 9800, not in any raw Excel file)
- 88 truly missing patients are NOT in any of the 8 raw Excel files; research_ids concentrated in 9800-11220 range; appear only in operative_details or molecular_testing tables as orphan records
- `us_dominant_nodule_size_v1` table (3,440 rows): per-patient dominant thyroid nodule size (cm); source priority: imaging_excel > tirads_structured > nlp_detail_text; 3,439 from `raw_us_tirads_excel_v1` (COMPLETE_MULTI_SHEET_ULTRASOUND_REPORTS.xlsx per-nodule measurements), 1 from NLP; median 1.9cm, range 0.3-4.3cm (structured), 0.2-12.5cm (NLP)
- `serial_imaging_us.dominant_nodule_size_on_us` is entirely NULL (0/18,753 rows); source Excel column also empty; use `us_dominant_nodule_size_v1` as replacement
- Script 26 MATERIALIZATION_MAP expanded to 135 entries: adds `md_stg_dob_excel_recovery`, `md_us_dominant_nodule_size_v1`, `md_demographics_harmonized_v2`, `md_qa_missing_demographics`
- Demographics harmonized coverage (final): age 99.2% (11,585/11,673), sex 93.2%, race 93.1%; age sources: benign_pathology 10,871 + excel_dob_unanimous 714 + missing 88
- Analysis-grade resolved layer (scripts 48-55, 2026-03-13): new pipeline adds unified manuscript-ready resolved cohort, enhanced linkage v3 with numeric scores, multi-nodule imaging support, formalized thyroid scoring systems, structured complication phenotyping, longitudinal lab hardening, local DuckDB verification reports, and assertion-based validation suite
- `patient_analysis_resolved_v1`: ONE row per patient (~12,886); merges all extraction/refinement/adjudication/linkage/scoring into single manuscript-grade wide table; columns follow {domain}_{var}_raw/_final/_source/_confidence pattern; eligibility flags: analysis_eligible_flag, molecular_eligible_flag, rai_eligible_flag, survival_eligible_flag, scoring_ajcc8/ata/macis_flag; audit cols: resolved_layer_version='v1', source_script='48'
- `episode_analysis_resolved_v1`: one row per surgery episode; includes linked pathology/FNA/RAI with v2 and v3 linkage scores
- `lesion_analysis_resolved_v1`: one row per tumor/lesion from tumor_episode_master_v2; includes linked FNA, molecular, imaging nodule with v3 linkage scores
- `thyroid_scoring_systems_v1`: one row per patient; AJCC8 T/N/M/stage_group (DTC age-dependent rules), ATA 2015 initial risk (low/intermediate/high), ATA response-to-therapy, MACIS score+risk_group, AGES score, AMES risk classification, LN burden metrics (ln_ratio, ln_burden_band), molecular risk composite (high/intermediate/low); every score has *_calculable_flag and *_missing_components columns; never silently computed when required inputs missing
- AJCC8 rules: T1a ≤1cm, T1b ≤2cm, T2 ≤4cm, T3a >4cm (no ETE), T3b gross ETE to perithyroidal; microscopic ETE does NOT upstage T1-T2 per AJCC8; age <55: Stage I (M0), Stage II (M1); age ≥55: I=T1-2/N0/M0, II=T1-2/N1 or T3, III=T4a or N1b, IVA=T4b, IVB=M1
- ATA 2015 initial risk: high=gross ETE/incomplete resection/distant mets/LN>3cm/FTC+extensive vascular; intermediate=aggressive variant/vascular invasion/microscopic ETE/>5LN or LN 0.2-3cm; low=all other PTC
- MACIS formula: 3.1×(age_factor) + 0.3×size + 1.0×(incomplete resection) + 1.0×(local invasion) + 3.0×(distant mets); age_factor=0.08×age if <40, 0.22×age if ≥40; risk groups: <6=low, 6-6.99=intermediate, 7-7.99=high, ≥8=very high
- `imaging_nodule_master_v1`: long-format per-nodule per-exam unpivoted from raw_us_tirads_excel_v1 (up to 14 nodule groups); columns: nodule_id (SHA), exam_id, tirads_reported, tirads_acr_recalculated, composition, echogenicity, shape, margins, calcifications, length/width/height_mm, max_dimension_cm, laterality, tirads_concordant_flag, suspicious_flag
- `imaging_exam_master_v1`: per-exam aggregation; `imaging_patient_summary_v1`: per-patient summary including multifocal_flag, bilateral_disease_flag, worst_tirads_category
- Enhanced linkage v3 tables (imaging_fna/fna_molecular/preop_surgery/surgery_pathology/pathology_rai_linkage_v3): add numeric linkage_score (0.0-1.0) and analysis_eligible_link_flag (score≥0.5); backward-compatible linkage_confidence_tier; `linkage_ambiguity_review_v1` surfaces all multi-candidate linkages
- Linkage scoring: temporal (0.50 weight) + laterality (0.30) + size compatibility (0.20 where applicable) - ambiguity penalty (0.0/0.1/0.2 for 1/2/3+ candidates); tier mapping: ≥0.85=exact_match, ≥0.65=high_confidence, ≥0.45=plausible, >0=weak
- `complication_phenotype_v1`: long-format per-patient per-entity; explicitly classifies note_mention_flag/suspected_flag/confirmed_flag/transient_flag/permanent_flag/surgery_related_flag/historical_only_flag/timing_window/final_complication_status/analysis_eligible_flag
- Hypocalcemia/hypoparathyroidism classification: biochemical_only (PTH<15 or Ca<8.0 within 30d, no treatment), treatment_requiring (calcium/calcitriol within 60d), confirmed_transient (resolves <6mo), confirmed_permanent (persistent >6mo)
- `complication_patient_summary_v1`: wide per-patient flags; *_status per entity = confirmed_permanent/confirmed_transient/probable/absent/unknown; sources: extracted_complications_refined_v5 + extracted_rln_injury_refined_v2 + extracted_postop_labs_expanded_v1 + note_entities_medications
- `longitudinal_lab_clean_v1`: long-format per-lab; date precedence: specimen_collect_dt > entity_date > note_date (consistent with script 46); deduplicates same-day same-value duplicates; plausibility guards per lab type; `is_below_threshold` flag for "<0.2" censored values
- `longitudinal_lab_patient_summary_v1`: per-patient Tg/anti-Tg/TSH/PTH/Ca summaries with nadir, peak, last_value, rising_flag, doubling_time_days, lab_completeness_score (0-100)
- `recurrence_event_clean_v1`: structural (from extracted_recurrence_refined_v1) vs biochemical (rising Tg >1.0 and >2×nadir) recurrence events; source-linked with recurrence_definition column
- `val_scoring_systems`: scoring calculability summary table (% calculable per system, stage/risk distributions)
- `val_analysis_resolved_v1`: validation test results from script 55 (12 test groups, 20+ assertions); PASS/FAIL/SKIP per test; integrates with script 29 validation engine
- Script 26 MATERIALIZATION_MAP expanded to 163 entries (was 135): adds 28 new md_* entries for scripts 48-53 outputs
- Script 55 validation suite tests: duplicate patient IDs, episode uniqueness, linkage_score bounds [0,1], scoring NULL when calculable_flag=FALSE, complication timing plausibility, lab result plausibility bounds, date precedence, analysis eligibility consistency, AJCC8 T stage logic, molecular risk tier logic
- Deployment order updated: script 15 → ... → 47 → 51 → 50 → 49 → 52 → 53 → 48 → 26 --md → 54 --md → 55 --md
- `docs/analysis_resolved_layer.md` created: full documentation of resolved layer, column naming convention, linkage scoring formulas, scoring system formulas, complication definitions, lab date precedence, deployment instructions, known limitations
- Complex multi-CTE SQL on local DuckDB can hang indefinitely (10+ min) even for moderate data sizes; prefer pull-to-pandas + compute locally + push-via-parquet strategy for any SQL with >5 CTEs or >3 JOINs against local DuckDB
- local DuckDB transaction locks from long-running CREATE TABLE statements persist even after the Python process is killed; the only resolution is to wait (can be 15+ min) or write to an alternative table name and create a VIEW alias
- `thyroid_scoring_py_v1` is the Python-built scoring table (identical schema to `thyroid_scoring_systems_v1`); use when canonical name is locked; `_TABLE_FALLBACK` in `utils/statistical_analysis.py` maps canonical → fallback names
- Script 51 original SQL has multiple schema mismatches vs local DuckDB reality: `recurrence_confirmed` (should be `recurrence_flag_structured`/`recurrence_any`), `bethesda_confidence` (should be `best_source_reliability`), `bethesda_final` is BIGINT not VARCHAR, `histology_normalized` does not exist in mcv12, `surg_date` needs TRY_CAST for VARCHAR dates, `LIKE ANY (...)` requires square brackets `[...]` in DuckDB
- Script 51b (`51b_thyroid_scoring_python.py`): Python-based scoring alternative that bypasses the SQL approach; computes AJCC8, ATA, MACIS, AGES, AMES, LN burden, molecular risk in pandas; ~5s total runtime vs 10+ min stuck SQL
- `extracted_postop_labs_expanded_v1` uses `value` (not `result_numeric`) and `lab_date` (not `collection_date`); these column name mismatches were present in scripts 52 and 53
- `extracted_rln_injury_refined_v2` uses `rln_injury_tier` (not `tier`), `rln_injury_evidence_strength` (not `confidence_level`), `classification` (not `source_label`), `rln_injury_is_confirmed` (not computed from `tier IN (1,2)`)
- `extracted_recurrence_refined_v1` on local DuckDB has `recurrence_flag_structured`, `recurrence_any` (booleans), `recurrence_site_inferred`, `detection_category` — NOT `recurrence_confirmed`, `recurrence_site`, `recurrence_detection_method`
- `extracted_fna_bethesda_v1` on local DuckDB: `bethesda_final` is BIGINT (1-6 numeric), `best_source_reliability` (not `bethesda_confidence`), `source_tables` (not `bethesda_source`); CASE WHEN comparisons must use integers not strings
- Lesion JOIN in script 48 originally used direct `fna_episode_master_v2` JOIN by research_id + laterality + 365-day window → produced 105M-row cartesian product (11,691 tumors x 59,620 FNA); fix: join via `preop_surgery_linkage_v3` (one FNA per surgery via score_rank=1)
- `preop_surgery_linkage_v3`, `surgery_pathology_linkage_v3`, `pathology_rai_linkage_v3`, `fna_molecular_linkage_v3` initially created without `score_rank` column; added via ALTER TABLE post-creation
- When writing pandas DataFrames to local DuckDB, use parquet intermediary: `df.to_parquet(tmp); con.execute(f"CREATE TABLE x AS SELECT * FROM read_parquet('{tmp}')")` — avoids transaction conflicts from `con.register()` approach
- `pd.NA` / `NAType.__bool__` is a recurring issue when DuckDB nullable boolean/integer columns are used in Python row-wise functions; always use `val is True` pattern (not `val or False`, not `bool(val)`)
- Readiness gate results (2026-03-13): G1 PASS (0 patient dupes), G2 CONDITIONAL (146 episode dupes = multi-pathology-per-surgery), G3 PASS (AJCC8 98.6%, ATA 76.0% among eligible), G4 PASS (7 confirmed complication entity types), G5 PASS (all supporting tables non-empty), G6 PASS (data quality audit), G7 PASS (SAP exists)
- Scoring calculability among analysis-eligible patients: AJCC8 98.6%, ATA 76.0%, MACIS ~98%, AGES 100%, AMES 100%
- 146 episode-level duplicates in `episode_analysis_resolved_v1` are true multi-pathology-per-surgery edge cases; resolve with `QUALIFY ROW_NUMBER() OVER (PARTITION BY research_id, surgery_episode_id ORDER BY t_stage DESC) = 1` for episode-level modeling
- Phase 5 scripts: `57_freeze_manuscript_cohort.py` (creates `manuscript_cohort_v1` + `manuscript_cohort_flow_v1`), `58_missingness_summary.py`, `59_dataset_summary.py`, `60_outlier_scan.py` (creates `manuscript_outlier_flags_v1`), `61_table1_preview.py`
- Phase 6 scripts: `62_run_primary_descriptives.py` (Tables 1-3), `63_run_primary_models.py` (4 logistic models), `64_run_survival_analyses.py` (KM + Cox PH via lifelines), `65_generate_manuscript_tables.py` (markdown + LaTeX), `66_generate_manuscript_figures.py` (5 figures, 300 DPI)
- `docs/statistical_analysis_plan_thyroid_manuscript.md`: 909-line SAP covering 9 cohorts, 8 models, 8 sensitivity analyses, MICE protocol, diagnostics
- `docs/pre_manuscript_analysis_checklist.md`: 7-gate readiness checklist; scripts 49-53 must be materialized before script 48; scoring tables must be non-NULL before downstream analyses
- Manuscript output directories: `exports/manuscript_cohort_freeze/`, `exports/manuscript_analysis/`, `exports/manuscript_tables/`, `exports/manuscript_figures/`
- Deployment order for manuscript pipeline: 51b → 50 → 49 → 52 → 53 → 48 → 57 → 58-61 → 62-66
- Final readiness: CONDITIONALLY READY; only blocking condition is G2 (146 episode dupes, documented as non-error)
- Episode deduplication (2026-03-13): `episode_analysis_resolved_v1_dedup` (9,368 rows, 0 duplicates); dedup priority: analysis_eligible > t_stage severity > n_stage severity > tumor_size > ln_positive > path_link_score_v3 > path_link_confidence_v2; excluded rows in `episode_duplicate_review_v1` (207 rows)
- `episode_duplicate_profile_v1` (146 rows): classifies duplicate cause — 144 `other_join_fanout` (identical rows from v2 linkage fan-out), 2 `join_artifact_v2_linkage` (different v2 confidence tiers); duplicate counts: 114 groups of 2, 22 of 3, 4 of 4, 2 of 5, 3 of 8, 1 of 9
- Readiness gate results FINAL (2026-03-13): ALL 7 GATES PASS — G1 0 patient dupes, G2 0 episode dupes (dedup), G3 AJCC8 37.6% / MACIS 37.5% / AMES 100% / AGES 100%, G4 7 complication entities, G5 all 15 supporting tables populated, G6 0 null research_ids, G7 SAP exists; OVERALL STATUS = READY
- Analysis views on local DuckDB: `analysis_patient_v1` (VIEW → patient_analysis_resolved_v1), `analysis_episode_v1` (VIEW → episode_analysis_resolved_v1_dedup), `analysis_lesion_v1` (VIEW → lesion_analysis_resolved_v1)
- Pre-computed analysis subsets: `analysis_cancer_cohort_v1` (4,136 rows, analysis-eligible + scoring), `analysis_tirads_subset_v1` (3,474 rows), `analysis_molecular_subset_v1` (10,025 rows), `analysis_recurrence_subset_v1` (1,946 rows)
- `manuscript_cohort_v1` has 139 columns (126 original + 8 aliases + 5 metadata); aliases: surgery_date→surg_first_date, path_ete_final→ete_grade_final, mol_braf_positive_final→braf_positive_final, mol_tert_positive_final→tert_positive_final, path_multifocal_final→path_multifocal_flag, path_age_at_surgery_raw→age_at_surgery, demo_sex_final→sex, demo_race_final→race
- Statistical analysis results (2026-03-13): Table 1 (144 rows demographics), Table 2 (78 rows tumor/treatment), Table 3 (8 rows outcomes); RAI logistic model C: stage III/IV OR=2.86 (1.07-7.62) p=0.035, tumor_size OR=1.18 (1.05-1.34) p=0.007; survival cohort N=3,201, 965 events, median 7.4y follow-up; Cox PH concordance=0.853
- KM survival by AJCC8: 5y I/II=0.823, III/IV=0.161 (log-rank p<0.0001); by BRAF: 5y BRAF+=0.565, BRAF-=0.753 (p<0.0001); by ATA: 5y high=0.504, intermediate=1.0, low=1.0
- Cox PH Schoenfeld non-proportionality: age (p=0.024), stage_III_IV (p<5e-5), ata_high (p<5e-5), ln_positive (p=0.042); ata_intermediate near-perfect separation warning
- Script 64 bug fix: `build_km_summary()` used `surv.loc[mask]` instead of `sub.loc[mask]` causing index alignment AssertionError
- Script 66 bug fix: `figure_stage_distribution()` used `df.get("Variable", pd.Series(dtype=str))` which creates an unalignable index; replaced with explicit `"Variable" in df.columns` check
- `path_ln_positive_raw` is INTEGER count (0, 1, 2, ...) not boolean; scripts using `_to_bool_int()` on this column lose all values >1 (become NaN); for logistic models, convert with `(col > 0).astype(int)` instead
- FINAL_PUBLICATION_BUNDLE_20260313: 62 files including manuscript_cohort_v1.csv, episode_dedup_v1.csv/parquet, master_clinical_v12.csv/parquet, 5 figures (PNG+SVG), 7 tables (CSV+MD+TEX), 4 analysis CSVs, readiness_assessment.json, manifest.json
- Script 70 (`70_canonical_backfill.py`): targeted idempotent backfill of canonical episode tables from refined/linkage sidecar tables; supports `--md`; creates `canonical_backfill_ambiguity_review` table for multi-candidate linkages; exports audit artifacts to `exports/canonical_backfill_YYYYMMDD_HHMM/`
- Canonical backfill results (2026-03-13): RAI dose 55→371/1,857 (20.0%), RAS flag 0→325/10,126 (3.2%), molecular→FNA link 0→639, FNA→molecular link 0→689, RAI→surgery link 0→19, imaging→FNA link 0→0 (source empty); total 1,988 cells backfilled
- `imaging_fna_linkage_v3` has 0 rows on local DuckDB — imaging nodule size data not populated in structured tables; `imaging_nodule_long_v2.linked_fna_episode_id` cannot be backfilled until upstream imaging data is resolved
- Some V3 linkage tables have duplicate `score_rank` and `score_rank_1` columns (from ALTER TABLE post-creation); both contain identical values; use `score_rank` as the canonical column
- DuckDB UPDATE ... SET ... FROM with multiple matching rows picks an arbitrary match; always deduplicate source CTE with ROW_NUMBER before UPDATE to ensure determinism
- `extracted_rai_dose_refined_v1` has 1 duplicate on (research_id=7332, rai_date=2019-10-16); dedup via `ROW_NUMBER() OVER (PARTITION BY research_id, rai_date ORDER BY source_reliability DESC, dose_mci DESC)` in backfill script
- MATERIALIZATION_MAP now includes `episode_analysis_resolved_v1_dedup` and `manuscript_cohort_v1` (added 2026-03-13); standard `scripts/26_local DuckDB_materialize_v2.py --md` refresh path covers the full resolved+manuscript layer
- Script 76 (`76_canonical_gap_closure.py`): 5-phase canonical gap closure — operative NLP enrichment (6 new columns), RAI dose provenance + relaxed backfill (20%→41%), molecular RAS subtype propagation (134 rows), linkage ID propagation (6 canonical tables), recurrence date hardening (4 tiers); supports `--md`, `--local`, `--dry-run`, `--phase A/B/C/D/E/all`
- Script 77 (`77_lab_canonical_layer.py`): forward-compatible canonical lab scaffold; creates `longitudinal_lab_canonical_v1` (45,954 rows, 5 analytes, 3,349 patients) and `val_lab_completeness_v1` (14 analyte entries with completeness tiers); supports `--md`, `--local`
- `longitudinal_lab_canonical_v1`: unified long-format lab table; schema: research_id, lab_date, lab_date_status, lab_name_raw/standardized, analyte_group, value_raw/numeric, unit_raw/standardized, is_censored, source_table/script, ingestion_wave, data_completeness_tier; data_completeness_tier values: current_structured / current_nlp_partial / future_institutional_required
- `val_lab_completeness_v1`: 14 entries; 5 populated analytes (thyroglobulin, anti_tg, pth, calcium_total, calcium_ionized) + 9 future placeholders (tsh, free_t4, free_t3, vitamin_d, albumin, phosphorus, magnesium, calcitonin, cea)
- RAI dose post-backfill: 761/1,857 (41.0%) episodes have dose_mci; dose_source and dose_confidence columns added; `surgery_link_score_v3` from `pathology_rai_linkage_v3` propagated to 70 episodes
- Molecular RAS subtype: 325/10,126 (3.2%) episodes have ras_subtype filled; `fna_link_score_v3` from `fna_molecular_linkage_v3` propagated to 693 episodes
- Linkage propagation results: `tumor_episode_master_v2` 80.4% surgery-linked (9,401/11,691); `operative_episode_detail_v2` 93.2% path-linked + 33.9% FNA-linked; `fna_episode_master_v2` 5.6% surgery-linked (3,339/59,620); imaging→FNA still 0% (source empty)
- Recurrence date tiers: exact_source_date (54), biochemical_inflection_inferred (168), unresolved_date (1,764), not_applicable (8,885); `recurrence_date_best`, `recurrence_date_status`, `recurrence_date_confidence` columns added to `extracted_recurrence_refined_v1`
- `operative_episode_detail_v2` new columns (script 76 Phase A): `parathyroid_identified_count`, `parathyroid_resection_flag`, `frozen_section_flag`, `berry_ligament_flag`, `ebl_ml_nlp`, `op_enrichment_source` — all structurally in place but require V2 extractor output materialization to local DuckDB to populate
- `note_entities_procedures` on local DuckDB has only procedure-level entities (hemithyroidectomy, total_thyroidectomy, etc.); V2 OperativeDetailExtractor domain entities (parathyroid_management, specimen_detail, berry_ligament, etc.) are NOT materialized there — must run V2 extractors and upload to local DuckDB
- Dashboard restructured from 39 flat tabs to 6 workflow sections: Overview, Patient Explorer, Data Quality, Linkage & Episodes, Outcomes & Analytics, Manuscript & Export; each section contains nested sub-tabs
- New dashboard modules: `app/qa_workbench.py` (surfaces val_dataset_integrity_summary_v1, val_provenance_completeness_v2, val_episode_linkage_completeness_v1, val_lab_completeness_v1, actionable guidance), `app/manual_review_workbench.py` (chronology conflicts, extraction errors, linkage ambiguities, unresolved recurrence dates, review queue)
- Dashboard Overview now shows: manuscript cohort size (10,871), cancer cohort size (4,136), dedup episodes (9,368), linkage completeness, provenance completeness, chronology anomaly summary, lab coverage — all from health monitoring tables
- Patient Timeline Explorer enhanced: date-status color legend bar, eligibility badges (Histology/Molecular/RAI), source table metadata display per patient
...
- Script 80 (`80_structural_gap_maximization.py`): final structural-gap maximization pass across 4 domains (RAI, recurrence, non-Tg labs, operative semantics); creates 5 validation tables + 4 docs + exports bundle; supports `--md`, `--local`, `--dry-run`, `--phase A/B/C/D/E/all`
- `val_rai_structural_coverage_v1` (27 rows): comprehensive RAI coverage metrics — event-level (1,857 episodes, 434 exact-date, 838 inferred, 585 unresolved), patient-level (862 with any episode, 35 confirmed, 249 with dose), dose missingness (761 available, 1,096 linkage_failed), note coverage (0 nuclear med notes = first-class structural limitation)
- `val_rai_source_limitation_v1` (5 rows): per-source-domain RAI availability classification; nuclear_medicine_reports and structured_rai_orders both rated ABSENT with `first_class_structural_limitation` tier; endocrine/DC/op notes rated PARTIAL
- Nuclear medicine reports: formally classified as `nuclear_medicine_reports_absent` source limitation in both val tables; this is why RAI dose coverage is capped at ~41% and cannot improve without institutional nuclear medicine data feed
- `val_recurrence_readiness_v1` (10 rows): manuscript readiness separating total_patients (10,871), recurrence_any_flagged (1,986), date_exact_source (54, 2.7% — YES for TTE), date_biochem_inferred (168, 8.5% — CONDITIONAL), date_unresolved (1,764, 88.8% — NO for TTE), source_linked_multi (1,072, 54.0% — HIGH CONFIDENCE)
- Recurrence 88.8% unresolved-date rate is a source limitation: most structural recurrence flags come from `recurrence_risk_features_mv.recurrence_flag` (boolean without day-level date); without a structured recurrence registry, day-level dates cannot be recovered
- `val_lab_temporal_truth_v1` (14 rows): per-analyte temporal truth with `analysis_suitability` classification — thyroglobulin/anti_tg = `time_to_event_eligible` (100% date coverage), pth = `limited_temporal_fidelity` (17.4%), calcium_total = `limited_temporal_fidelity` (11.6%), calcium_ionized = `value_only_no_temporal`, tsh/free_t4/free_t3/vitamin_d/albumin/phosphorus/magnesium/calcitonin/cea = `no_data_source_absent`
- Non-Tg lab temporal truth: PTH and calcium dates are NLP-extracted or note-anchored only (`extracted_date` status); NOT suitable for precise postoperative-day analysis but acceptable for broad temporal windowing (0-30d vs 31-180d)
- `val_operative_field_semantics_v1` (17 rows): per-field parse status audit; 10 boolean fields classified as NOT_PARSED (FALSE = hardcoded default from script 22, NOT confirmed negative), 4 fields as SOURCE_ABSENT (berry_ligament, frozen_section, ebl_nlp, parathyroid_count — NLP entity types not in vocabulary), 2 as RELIABLE (central/lateral neck dissection — structured), 1 as PARTIAL (ebl_ml structured)
- Operative NOT_PARSED semantics: when `rln_monitoring_flag`, `parathyroid_autograft_flag`, `gross_ete_flag`, `local_invasion_flag`, `tracheal_involvement_flag`, `esophageal_involvement_flag`, `strap_muscle_involvement_flag`, `reoperative_field_flag`, `drain_flag`, `parathyroid_resection_flag` = FALSE in operative_episode_detail_v2, this means UNKNOWN/NOT_PARSED, not confirmed-negative; V2 extractor output was never materialized via COALESCE(new, old) where old=FALSE prevents overwrite
- MATERIALIZATION_MAP expanded to include 5 structural gap maximization tables (script 80); total now includes val_rai_structural_coverage_v1, val_rai_source_limitation_v1, val_recurrence_readiness_v1, val_lab_temporal_truth_v1, val_operative_field_semantics_v1
- Deployment order updated: ... → 78 → 80 → 26 --md
- Exports at `exports/structural_gap_maximization_20260313_1411/`: 5 CSVs + manifest.json
- Docs: `docs/rai_structural_gap_maximization_20260313.md`, `docs/recurrence_structural_gap_maximization_20260313.md`, `docs/non_tg_lab_temporal_truth_audit_20260313.md`, `docs/operative_semantics_hardening_20260313.md`
- Manuscript Freeze Package (2026-03-13): `docs/MANUSCRIPT_FREEZE_PACKAGE_20260313.md` master index linking all frozen manuscript components — cohort data, tables, figures, metric registry, caveats, supplement, reviewer defense snapshots
- Manuscript Metric Registry: `exports/manuscript_metric_registry_20260313/` — 25 canonical metrics in CSV + JSON + Markdown; single source of truth for all manuscript numbers; every metric has metric_id, canonical_value, numerator, denominator, denominator_label, source_table, source_script, sql_fragment, caveats, frozen_date
- Primary denominators: full_surgical_cohort=10,871, analysis_eligible_cancer=4,136, molecular_tested=10,025, survival_cohort=3,201, vascular_positive=3,846
- Manuscript Caveats Pack: `docs/MANUSCRIPT_CAVEATS_20260313.md` — 8 pre-written caveats with Methods/Limitations/Discussion wording: (1) non-Tg lab dates, (2) recurrence date sparsity, (3) nuclear medicine absence, (4) partial note coverage, (5) vascular present_ungraded, (6) operative boolean defaults, (7) BRAF prevalence context, (8) scoring calculability
- Supplementary Data Quality Appendix: `docs/SUPPLEMENT_DATA_QUALITY_APPENDIX_20260313.md` — 9-section journal supplement covering cohort derivation, data sources/linkage, date quality taxonomy, domain coverage, structural gaps, validation gates, NLP extraction quality, scoring calculability, reproducibility
- Reviewer Defense Snapshots: `docs/reviewer_defense_20260313/` — 6 compact QA docs: (01) duplicate prevention, (02) recurrence definition/dating, (03) operative detail sourcing, (04) imaging/TIRADS completeness, (05) RAI receipt/dose, (06) notes vs structured sourcing
- Script 90 (`90_manuscript_freeze_rebuild.py`): one-command manuscript publication bundle rebuild; validates 16 source tables with expected row counts (1% drift tolerance), uniqueness constraints on patient/episode tables, fail-closed (exit 1) on critical table missing or drifted; regenerates Tables 1-3, figures, metric registry, readiness assessment, and ZIP bundle; supports `--md`, `--dry-run`
- Script 90 critical tables (fail-closed): manuscript_cohort_v1 (10,871), episode_analysis_resolved_v1_dedup (9,368), patient_analysis_resolved_v1 (10,871), thyroid_scoring_py_v1 (10,871), analysis_cancer_cohort_v1 (4,136)
- Rebuild command: `.venv/bin/python scripts/90_manuscript_freeze_rebuild.py --md` (full rebuild) or `--dry-run` (validate only)
- Script 86 (`86_operative_nlp_final_sync.py`): operative NLP propagation fix — adds 7 missing episode-level NLP columns to `episode_analysis_resolved_v1` via composite-join UPDATE; adds 11 `op_*` patient-level aggregates to `patient_analysis_resolved_v1` and `manuscript_cohort_v1`; rebuilds dedup and md_ mirrors; creates `val_operative_nlp_final_sync_v1` (28 rows) on local DuckDB; supports `--md`, `--phase A–J`
- `surgery_episode_id` is per-patient sequential integer (1, 2, ... per patient) — NOT globally unique across patients; always use composite key `(research_id, surgery_episode_id)` for episode-table JOINs; single-field join over 9,371×9,575 rows produces ~89.7M-row cartesian product
- DuckDB UPDATE SET clause requires bare (unqualified) column names on left side: `col = expr` correct; `table.col = expr` raises BinderException; cannot COALESCE target table's current column value in SET (no self-reference) — use `COALESCE(src.col, FALSE)` not `COALESCE(src.col, target.col, FALSE)`
- DuckDB DDL (`ALTER TABLE ADD COLUMN`, `CREATE OR REPLACE TABLE`) returns rowcount=-1; this is success not error; guard with `if rc == -1: rc = 0` in any wrapper that checks rowcount for pass/fail
- `episode_analysis_resolved_v1` now has 7 operative NLP columns (added script 86): `parathyroid_autograft_flag`=40, `local_invasion_flag`=25, `tracheal_involvement_flag`=9, `esophageal_involvement_flag`=0 (source-limited), `strap_muscle_involvement_flag`=186, `reoperative_field_flag`=49, `operative_findings_raw`=594 non-null
- `patient_analysis_resolved_v1` and `manuscript_cohort_v1` now have 11 `op_*` patient-level aggregates (script 86): `op_rln_monitoring_any`=1701, `op_drain_placed_any`=169, `op_strap_muscle_any`=186, `op_reoperative_any`=46, `op_parathyroid_autograft_any`=40, `op_local_invasion_any`=25, `op_tracheal_inv_any`=9, `op_esophageal_inv_any`=0, `op_intraop_gross_ete_any`=22, `op_n_surgeries_with_findings`=587, `op_findings_summary`=587
- `esophageal_involvement_flag`=0 confirmed SOURCE_LIMITED (not a propagation bug): 0 entity records in `note_entities_procedures` for esophageal type; 10 op notes matching the SQL pattern are anatomical references/intact language; not re-engineered
- Use `git commit -F msgfile` for multiline commit messages in zsh to avoid getting the shell stuck in dquote mode; run `exec zsh` to reset a terminal stuck in dquote state
- Script 96 (`96_episode_downstream_repair.py`): downstream episode_id propagation fix; temporal-matches OED/EARD/v3-linkage rows against canonical `tumor_episode_master_v2` (research_id, surgery_date) → surgery_episode_id mapping; supports `--md`, `--dry-run`; creates `episode_downstream_repair_audit_v1` provenance table; auto-generates markdown report + JSON metrics
- Script 97 (`97_episode_linkage_audit.py`): multi-surgery episode linkage integrity audit; creates 7 tables in target env (`multi_surgery_episode_cohort_v1`, `val_episode_key_propagation_v1`, `val_episode_artifact_assignment_v1`, `val_episode_ambiguity_review_v1`, `val_episode_mislink_candidates_v1`, `val_episode_health_scorecard_v1`, `val_episode_summary_kpi_v1`); supports `--env dev/qa/prod`
- Script 95 (`95_episode_linkage_repair.py`): 8-phase (A–H) episode-aware linkage repair covering tumor, operative, pathology, RAI, preop, imaging, molecular, and FNA domains
- Episode downstream repair results (2026-03-15): 434 total rows repaired across 5 tables — OED 96, EARD 96, surgery_pathology_linkage_v3 200, pathology_rai_linkage_v3 4, preop_surgery_linkage_v3 38; non-regression PASS
- OED `surgery_date_native` (DATE) is the reliable date column for episode matching; `resolved_surgery_date` (VARCHAR) is unreliable and must NOT be used for temporal joins
- `surgery_pathology_linkage_v3` uses `surg_date` column (NOT `surgery_date`); confirmed via BinderException during development
- All 5 downstream tables already had `surgery_episode_id` BIGINT column (no ALTER TABLE needed); they were populated with all-1s before repair
- OED no_date_match gap: 647 rows — 635 patients NOT in tumor_episode_master_v2 (benign/non-cancer), 11 date mismatches; all correctly remain ep_id=1
- Multi-surgery "all-ones" gap: 525 of 761 multi-surgery patients have only 1 OED row; their 2nd+ surgeries exist in tumor_episode but NOT in operative_episode_detail_v2 — data population gap, not propagation bug
- `tumor_episode_master_v2` has multiple rows per (research_id, surgery_date) due to multi-tumor patients; MUST use `ROW_NUMBER() OVER (PARTITION BY research_id, surgery_date ORDER BY tumor_ordinal)` to build canonical ep_id map; raw row count 11,691 vs 11,685 after dedup
- Post-repair audit KPIs: mislinks 1,464→1,506 (+42 = improved detection from corrected episode routing, not regression)
- local DuckDB environments: `thyroid_master.duckdb` (prod), `thyroid_master.duckdb` (dev), `thyroid_master.duckdb` (qa); dev has only audit tables from script 97; prod has all 18+ episode linkage tables and 130+ other tables
- `.streamlit/secrets.toml` does NOT exist at workspace path; LOCAL_DB_PATH must be obtained from environment variable only (exported in `~/.zshrc`); background terminal sessions need explicit `export LOCAL_DB_PATH="..."` since they don't source `.zshrc`
- Script 99 (`99_comprehensive_final_verification.py`): 5-workstream comprehensive verification against local DuckDB prod; creates `val_multi_surgery_cohort_v3` and `val_multi_surgery_review_queue_v3` tables; outputs 5 docs + 3 export dirs; supports `--md` (default), `--local`, `--dry-run`
- Script 99 canonical truth metrics (2026-03-15): 10,871 patients, 4,136 cancer, 9,368 dedup episodes, 761 multi-surgery (1,576 episodes), 1,986 recurrence, 1,857 RAI episodes, 629 local DuckDB tables, 39,961 canonical lab rows, RAI dose 41.0%, BRAF=546, RAS=292, TERT=96, TIRADS=3,474
- Script 99 operative NLP propagation: 4 PIPELINE_GAP fields exist in upstream `operative_episode_detail_v2` but NOT in downstream analytic tables — `rln_monitoring_flag` (1,702 rows), `rln_finding_raw` (371), `gross_ete_flag` (22), `drain_flag` (169); script 86 propagated only 7 of ~15 fields; 5 SOURCE_LIMITED fields have 0% upstream (esophageal_involvement, parathyroid_identified_count, frozen_section, berry_ligament, ebl_ml_nlp)
- Script 99 recurrence review: 3,032 cases exported in 31 CSV batches to `exports/recurrence_review_packets/`; priority tiers: 2,557 unresolved, 303 biochemical, 172 exact; 1,362 multi-surgery patients included
- Script 99 doc reconciliation: 1 cosmetic mismatch — `docs/MANUSCRIPT_CAVEATS_20260313.md` says "88.8%" for recurrence unresolved, live calculation 1764/1986=88.82% rounds to "89%" at integer precision; documented value is more precise/correct
- Script 99 multi-surgery review queue: 334 medium-priority entries (patients with 2+ surgeries and potential mislinks); stored in `val_multi_surgery_review_queue_v3` on local DuckDB prod
- Script 99 deliverables: `docs/dataset_truth_snapshot_20260315.md`, `docs/multi_surgery_linkage_audit_20260315.md`, `docs/episode_linkage_rulebook_20260315.md`, `docs/operative_nlp_propagation_audit_20260315.md`, `docs/recurrence_review_status_20260315.md`; exports in `exports/dataset_truth_snapshot_20260315/`, `exports/multi_surgery_linkage_audit_20260315_0202/`, `exports/recurrence_review_packets/`
- Script 100 (`100_episode_linkage_v2_hardening.py`): Phase 2 episode-aware linkage hardening for multi-surgery patients; 8 phases (A-H): (A) surgery-pathology re-scoring with proximity tiebreaker, (B) preop-surgery re-anchoring, (C) pathology-RAI re-anchoring, (D) downstream ep-id sync, (E) governed validation scorecard + review queue, (F) single-surgery non-regression proof, (G) before/after delta, (H) md_* mirror materialization; supports `--md`, `--local`, `--env dev/qa/prod`, `--phase A-H`, `--dry-run`
- Script 100 local DuckDB workspace mode: uses `thyroid_master.duckdb.main.` prefix for reading prod source tables in dev/qa; UPDATEs are prod-only (`--env prod`) and target bare table names; never uses ATTACH
- Script 100 v2 confidence tiers: exact_day (same-day match), anchored_window (within surgery windows), ambiguous_multi_surgery (near multiple surgeries), plausible_extended (wider window), weak_temporal (any temporal), unlinked (no date match)
- Script 100 conservative UPDATE gate: only applies UPDATEs where `ep_id_changed IS TRUE AND ambiguity_flag IS NOT TRUE AND v2_confidence_tier IN ('exact_day', 'anchored_window')`; in practice 0 rows qualified for UPDATE in 2026-03-15 prod run (all 3 changed rows were ambiguous)
- Script 100 output tables (8): `episode_linkage_v2_sp_rescored`, `episode_linkage_v2_preop_rescored`, `episode_linkage_v2_rai_rescored`, `episode_linkage_v2_downstream_sync`, `val_episode_linkage_v2_scorecard`, `val_episode_linkage_v2_review_queue`, `val_episode_linkage_v2_nonregression`, `val_episode_linkage_v2_delta`; all mirrored as `md_*` on local DuckDB
...
- Pre-manuscript GO/NO-GO assessment (2026-03-15): verdict **GO WITH CAVEATS**; live local DuckDB verification: 20/20 table row counts PASS, 0 duplicate patients, 7/7 readiness gates PASS, 5 source-limited caveats (all with pre-written language); report at `docs/pre_manuscript_go_no_go_20260315.md`, artifacts at `exports/pre_manuscript_go_no_go_20260315/`
- local DuckDB total tables: 668 (as of 2026-03-15 live query)
- `.streamlit/secrets.toml` does NOT exist at workspace path; all local DuckDB scripts must use `os.environ['LOCAL_DB_PATH']` for token access — do NOT attempt `toml.load('.streamlit/secrets.toml')`
...
- `longitudinal_lab_canonical_v1` analyte groups: `thyroid_tumor_markers` (38,566 rows, 2,578 pts), `parathyroid` (797 rows, 673 pts), `calcium_metabolism` (598 rows, 562 pts); total 39,961 rows, 3,349 patients (pre-Tg ingestion); after script 113: 76,971 Tg/TgAb rows appended from structured EHR lab feed
- Script 113 (`113_tg_lab_ingestion.py`): structured EHR thyroglobulin (Tg) + thyroglobulin antibody (TgAb) ingestion; source: `Thyroid_Thyroglobulin_Lab_20251120.csv` (78,112 rows, 3,298 patients); PII-stripped, deduplicated (106 removed), combo panel disambiguated (34,503 combo rows → 16,173 heuristic + 561 crossref + 607 ambiguous to review); output: 76,971 canonical + 1,035 review queue; 97.4% numeric parse rate, 100% date coverage; supports `--duckdb`, `--md`, `--dry-run`
- Script 113 Phase P: emits `processed/tg_lab_ingestion_qc_v1.json` (machine-readable QC artifact); fields: schema_version, run_timestamp, source_file, row_waterfall (source_rows/after_dedup/duplicates_suppressed/rows_appended_canonical/review_queue_rows/reconciliation_gap), patients, analyte_breakdown, combo_disambiguation, result_parsing, temporal_window_distribution, cross_wave_reconciliation, derived_views, promotion_gate; reconciliation_gap=0 is required before promotion
- Script 113 hardening (2026-04-03): (1) parquet append in Phase J now purges prior `source_script='113_tg_lab_ingestion'` rows before concat — idempotent on repeated runs matching DuckDB behavior; (2) `ingestion_date` uses fixed run-start `TIMESTAMP` constant for reproducibility; (3) Phase N recurrence linkage logs explicit WARNING + stats entry when no recurrence table found (no silent skip); (4) Phase H asserts provenance column completeness after schema alignment
- `tests/test_tg_lab_ingestion.py`: 51-test pytest suite for script 113; covers analyte classification (TEST_NAME_MAP), result parsing (8 flag types), temporal windows (6 windows + gap/sort checks), combo disambiguation (heuristic + crossref + ambiguous), dedup key stability (Phase C), provenance columns (Phase H), and review queue routing (Phase D); uses importlib to load the digit-prefixed script file
- `docs/tg_structured_lab_promotion_note.md`: integration note for Tg structured lab promotion; describes data flow (Phases A→P), canonical layer integration (4 ingestion waves + cross-wave priority), downstream recurrence surveillance (timeline/postop/linkage derived views), QC artifact field mapping to 8-gate promotion criteria, run commands (dry-run/local DuckDB/MotherDuck), and known limitations (8 unmatched IDs, TgAb interference, recurrence table dependency)
- `thyroglobulin_lab_canonical_v1` (76,971 rows, 3,258 patients): canonical Tg/TgAb lab table; schema: research_id, analyte (Tg/TgAb), assay_method (immunoassay/LC-MS/MS/RIA/IMA/comprehensive/IgG/combo_panel/reflex), test_name_raw, specimen_collect_dt, order_dt, result_raw, result_numeric, result_qualifier (</>=/null), result_flag, days_from_surgery, temporal_window, surg_date, race, gender, age_at_surgery, thyroid_procedure, disambiguation_method (direct_label/combo_heuristic/combo_crossref), disambiguation_confidence, ingestion_script, ingestion_date
- `tg_lab_review_queue_v1` (1,035 rows, 279 patients): ambiguous combo panel pairs + unmapped test names; columns: research_id, test_name_raw, specimen_collect_dt, result, review_reason (combo_ambiguous/unmapped_test_name)
- Combo panel disambiguation heuristic (99.2% validated accuracy): `<0.9` → TgAb; `<0.1`/`<0.2` → Tg; `<2`/`<2.0`/`<20` vs `<0.X` → larger is TgAb; cross-reference same-patient labeled results for remaining pairs
- 8 unmatched research_ids in Tg lab data: 20038, 20040, 20041, 20044, 20045, 20048, 20049, 20054 (not in master cohort)
...
- Script 101 (`101_multi_episode_linkage_hardening.py`): multi-episode linkage hardening for multi-surgery patients; 3 SQL phases (linkage quality, cross-episode contamination, ambiguity review); supports `--md`, `--dry-run`; creates 3 tables + 3 md_ mirrors on local DuckDB; exports CSV+Parquet+manifest to `exports/multi_episode_linkage_hardening_YYYYMMDD_HHMM/`; report to `docs/multi_episode_linkage_hardening_YYYYMMDD.md`
- `val_multi_episode_linkage_v1` (4,739 rows): per-(research_id, surgery_rank, domain) linkage status; columns: research_id, surgery_rank, n_surgeries, is_completion, reop_flag, domain (pathology/preop/rai), linkage_status (linked/ambiguous/unlinked), linkage_score, confidence_tier, n_candidates, n_domains_linked, avg_score, episode_quality_grade (GREEN/YELLOW/RED)
- `val_cross_episode_contamination_v1` (30 rows): artifacts linked to wrong surgery episode; columns: research_id, domain, artifact_date, current_surgery_date, current_episode_id, closest_surgery_date, closest_episode_id, day_diff_current, day_diff_closest, contamination_type (wrong_episode), severity (high/medium)
- `review_multi_episode_ambiguities_v1` (1,351 rows): ambiguous multi-candidate linkages + episode mismatches; columns: research_id, domain, ambiguity_type, n_candidates, best_score, second_best_score, score_gap, is_completion, reop_flag, priority (critical/high/medium/low), manuscript_impact_flag
- Multi-episode linkage findings (2026-03-15): 761 multi-surgery patients, 1,576 episodes; pathology linked 39.3%, preop 18.7%, RAI 0.2%; 30 cross-episode contamination cases (2 pathology, 28 preop); 1,351 ambiguity items (6 critical, 299 high); 1,289 manuscript-impacting; no manuscript numbers changed — ambiguous cases quarantined
- `multi_surgery_episode_cohort_v1` boolean columns (`is_completion`, `reop_flag`) stored as VARCHAR text in local DuckDB (can be empty string ''); must use safe CAST via `CASE WHEN LOWER(CAST(col AS VARCHAR)) IN ('true','1','yes','x') THEN TRUE ELSE FALSE END`
- Script 104 (`104_operative_truth_state_hardening.py`): 8-phase hardening (A–H) that recodes hardcoded FALSE→NULL in 10 operative boolean fields, re-propagates through episode/patient/manuscript layers, rebuilds md_ mirrors, creates validation tables, exports artifacts; supports `--md`, `--dry-run`, `--phase A-H|all`
- Operative boolean tri-state semantics (post-script 104): TRUE = NLP-confirmed positive, NULL = unknown/not parsed, FALSE = reserved for future explicit negation; never use COALESCE(field, FALSE) at storage/propagation boundary — only at display/scoring consumption boundary
- Script 86 COALESCE defects fixed (2026-03-15): `BOOL_OR(COALESCE(field, FALSE))` → `BOOL_OR(field)` in PATIENT_OP_FIELDS; `COALESCE(o.field, FALSE)` → bare `o.field` in UPDATE_EPISODE_SQL; prevents future re-introduction of FALSE contamination
- Script 22 FALSE-hardcoding is the ROOT CAUSE (line 643–656: `FALSE AS rln_monitoring_flag` etc.) but NOT modified — schema redesign required; script 104 corrects downstream
- `val_operative_truth_state_v1` (39 rows) and `val_operative_truth_state_delta_v1` (39 rows) on local DuckDB: per-field truth-state distributions and before/after delta across 4 tables
- Post-hardening operative_episode_detail_v2: all 10 boolean fields have 0 FALSE values; only TRUE and NULL; TRUE counts: rln_monitoring 1,702; strap_muscle 186; drain 169; reoperative 46; parathyroid_autograft 40; local_invasion 25; gross_ete 22; tracheal 9; esophageal 0; parathyroid_resection 0
- Bonus fix discovered in script 104: rln_monitoring_flag and drain_flag had TRUE=0 in episode_analysis_resolved_v1 BEFORE hardening despite TRUE>0 in OED — old COALESCE chain was masking TRUE propagation; now correctly propagated
- Scoring scripts (51, 51b) use `COALESCE(pt.gross_ete, FALSE)` for AJCC8 T3b — intentionally NOT changed; unknown ETE → NULL → COALESCE → FALSE → no T3b upstaging (clinically conservative and correct)
- DuckDB/local DuckDB UPDATE rowcount returns -1 (safe_exec converts to 0); use Phase G delta validation tables to verify actual changes, not in-flight rowcount
- Script 105 (`105_manuscript_freeze_v1.py`): 7-phase manuscript publication freeze — environment verify, table inventory, Parquet/CSV export, checksum, manifest, optional local DuckDB stamp, integrity verify; supports `--md`, `--dry-run`, `--stamp`, `--version TAG`, `--skip-data`
- Freeze table tiers: Tier 1 (11 tables, fail-closed) = manuscript_cohort_v1 (10,871), patient_analysis_resolved_v1 (10,871), episode_analysis_resolved_v1_dedup (9,368), lesion_analysis_resolved_v1 (11,851), thyroid_scoring_py_v1 (10,871), analysis_cancer_cohort_v1 (4,136), complication_phenotype_v1 (5,928), complication_patient_summary_v1 (2,892), recurrence_event_clean_v1 (1,946), longitudinal_lab_canonical_v1 (39,961), survival_cohort_enriched (61,134); Tier 2 (16 tables, warn) = supporting analysis views; Tier 3 (6 tables, info) = validation/QA
- Freeze v1 output: `exports/manuscript_freeze_v1/` — manifest.json, table_inventory.csv, rowcount_summary.json, metadata.json, verification_report.json, data/*.parquet + *.csv; 33 tables, 261,114 total rows, 70 files, 65/65 checksums PASS
- Freeze refresh strategy: re-run with `--version v2` after external lab/med data arrives; v1 bundle remains untouched; compare manifests to quantify drift
- `--stamp` creates `{table}_freeze_{version}` TABLE copies in local DuckDB prod for point-in-time audits
...
- **Multinodule strict-key vs policy (2026-04):** Comparing **strict** `research_id|exam_date|nodule_number` keys from scored / Imaging_12 workbooks to `imaging_nodule_master_v1` **without** the same ±30d dedup as `scripts/50_multinodule_imaging.py` yields apparent “misses” (e.g. 527 + 620). **Policy-aligned** evidence: `studies/20260413_full_execution_reaudit/us_nodule_coverage_audit_policy_aligned.csv` (`true_gap_after_30d_policy` = 0 for those corpora). See `studies/20260413_full_execution_reaudit/FAILURE_REMEDIATION_20260413.md`.
- **`serial_imaging_us` (MotherDuck):** Empty placeholder TABLE via `scripts/155_md_serial_imaging_us_placeholder.py --md` so canonical episode code does not hit a missing object; populate only from an institutional feed — do not invent rows.
- **FNA Bethesda — episode master vs analysis (B3):** `fna_episode_master_v2.bethesda_category` may be NULL while cytology supplies Bethesda. **SSOT for resolved numeric Bethesda:** `v_fna_episode_bethesda_resolved_v1` (`scripts/sql/source_truth_confirmation_v1.sql`, deploy `151_source_truth_confirmation_v1.py --md`). **Convenience VIEW** (episode vs resolved + bucket): `v_fna_bethesda_episode_vs_resolved_v1` from `scripts/sql/v_fna_bethesda_episode_vs_resolved_v1.sql`, deploy `scripts/156_md_bethesda_episode_vs_resolved_view.py --md` (after `151`). Do not silently UPDATE episode master without approved rules. See `studies/20260413_full_execution_reaudit/open_items_b3_b5.md`.
- **US cervical lymph nodes — structured detail (B5):** No per-level / laterality / size **table** exists; exam-level `ultrasound_reports.lymph_node_assessment` (where present) and heuristic audits apply. Manuscript claims must match available fields. See `studies/20260413_full_execution_reaudit/open_items_b3_b5.md`.
- **Canonical cleanup 2026-04-17 (run_id `canonical_cleanup_20260417`)**: scripts 272-277 + 274b applied phases 1–7 of the PROMPT_18/PART2 cross-validation cleanup; full audit at `FINAL_CANONICAL_CLEANUP_REPORT_20260417.md` and `studies/canonical_cleanup_20260417/*`.
- **MotherDuck row-count corrections (verified 2026-04-17):** `thyroglobulin_lab_canonical_v1` = **74,258** rows (prior PROMPT_18 cite of ~76,971 was stale); `operative_episode_detail_v2` = 9,371; `complication_phenotype_v1` = 5,978; `fna_episode_master_v2` = 8,119; `rai_treatment_episode_v2` = 1,857; `canonical_us_nodule_characteristics_v1` = 37,016; `synoptic_tumor_long_v1` = 11,103.
- **CPM `research_id` is VARCHAR**, while `complication_phenotype_v1` and `thyroglobulin_lab_canonical_v1` use BIGINT, and `operative_episode_detail_v2` / `rai_treatment_episode_v2` / `imaging_nodule_master_v1` / `canonical_us_nodule_characteristics_v1` use INTEGER. `fna_episode_master_v2.research_id` is VARCHAR. **Always CAST(other.research_id AS VARCHAR)** when joining the canonical spine.
- **CPM `max_tirads_ever` storage:** **BIGINT** (values 1-5), NOT 'TR1'-'TR5' VARCHAR. Any new transform must store integer; do not prefix `'TR'`.
- **New CPM columns added by canonical cleanup 2026-04-17:** `cpm_built_at` (TIMESTAMP), `comp_hypopara_permanent_source` (VARCHAR; values `lab_persistence` / `unknown_override`), `lateral_neck_dissected_structured_or_nlp` (BOOLEAN; preserves union of structured-OED + NLP). The Phase 1.8 `comp_hematoma_confirmed` / `comp_seroma_confirmed` / `comp_chyle_leak_confirmed` / `comp_wound_infection_confirmed` `IF NOT EXISTS` ADDs were no-ops (already present).
- **CPM build-provenance convention:** every reconciliation/cleanup run MUST (a) update `main.canonical_patient_master.cpm_built_at = CURRENT_TIMESTAMP` for all rows it touched and (b) insert one row into `manuscript_workspace.cpm_reconciliation_provenance_v1` (run_id, started_at, ended_at, phases_applied, critical/high/med findings cleared, holds).
- **Archive convention:** cross-DB archive moves from `thyroid_canonical_publication_v1_0.main` go to `"Thyroid 2026 UPdated".archive_pub_v1_0.<orig_name>_pre_<reason>_<UTC>` (NOT `archive_legacy`, NOT `.main`). 182 archive tables exist as of 2026-04-17.
- **manuscript_workspace canonical view count:** **67** views (was 65 pre-2026-04-17; +2 added by cleanup: `imaging_nodule_master_clean_v1`, `path_tumor_size_invariant_v1`). Plus manuscript_workspace tables added 2026-04-17: `cpm_hypopara_adjudication_queue_v1`, `lab_orphan_audit_v1`, `canonical_cleanup_audit_v1`, `cpm_reconciliation_provenance_v1`, `view_definitions_snapshot_bigcleanup`.
- **Phase 4.6 ajcc8_t_stage rename HELD as of 2026-04-17:** PRE-GATE found 9 cohort views with bare `ajcc8_t_stage` references; rename of `ajcc8_t_stage_corrected -> ajcc8_t_stage` (and old `ajcc8_t_stage -> ajcc8_t_stage_with_microete_t3b_DEPRECATED`) requires migrating those views first. See `studies/canonical_cleanup_20260417/ajcc8_t_stage_view_migration_proposal.md` and run `scripts/274b_canonical_cleanup_phase4_6_rename.py` only after the migration.
- **Hypopara contradiction adjudication queue:** `manuscript_workspace.cpm_hypopara_adjudication_queue_v1` holds patients where CPM `comp_hypoparathyroidism_permanent=TRUE` contradicts `complication_phenotype_v1.final_complication_status='confirmed_transient'`. Initial population (4 rids: 9765, 7487, 6447, 10743) inserted 2026-04-17. Never auto-reset; require manual review.
- **Tg-lab orphans (403 rids as of 2026-04-17):** `manuscript_workspace.lab_orphan_audit_v1` lists patients with rows in `thyroglobulin_lab_canonical_v1` but not in CPM. All 403 currently classified `likely_non_cancer` (zero evidence in fna_episode_master_v2 / tumor_episode_master_v2 / synoptic_tumor_long_v1 / path_synoptics / imaging_nodule_master_v1). Decision pending.
- **Phase 4.6 RENAME applied 2026-04-17 (commit `915a9a1`)**: `ajcc8_t_stage_corrected` was renamed to `ajcc8_t_stage`; old `ajcc8_t_stage` was renamed to `ajcc8_t_stage_with_microete_t3b_DEPRECATED`. The 9 manuscript_workspace.cohort_* views were rewritten to bare `ajcc8_t_stage` (now resolves to the corrected value). `ajcc8_t_stage_v2` was NOT touched (out of scope; intermediate column with distinct semantics — differs from the corrected value for ~1,085 patients and is consumed by 224/240/266b/266c). The PRE-GATE was a one-time go/no-go check, NOT a permanent migration requirement; bare `ajcc8_t_stage` references are now correct and expected.
- **`canonical_us_nodule_characteristics_v1` is NOT the same as `us_nodules_tirads`** — they're distinct tables. The 12/1/25 placeholder rids `2332, 2445, 7744` lived in `us_nodules_tirads` (now deleted; archived to `manuscript_workspace.us_nodules_tirads_placeholder_archive_v1`). Future placeholder hunts should check both tables.
- **Script 266b Gate B (relaxed 2026-04-17):** the original draft required BOTH `ajcc8_t_stage_corrected` absent AND `ajcc8_t_stage_v2` absent. Cleanup prompt only specified renaming `_corrected`; `_v2` is an unrelated historical intermediate. Gate B now requires only `_corrected` absent and `ajcc8_t_stage` present. `_v2` presence is informational. Documented in 266b's Phase 0 docstring + decision-log return value.
- **Phenotype rule observation (informational, 2026-04-17):** `complication_phenotype_v1` for hypoparathyroidism produces ZERO `confirmed_permanent` rows (only `absent_or_unconfirmed`, `confirmed_duration_unknown`, `probable`, `confirmed_transient`). Clinical 1-3% of thyroidectomies (~45-135 patients) expected; cohort has 0. Likely too-strict phenotype rule; surface at next clinical adjudication review.
- **266b ETE classifier — vocabulary tightening + Tier 4 CPM overlay (2026-04-17):** original keyword vocabulary missed 98% of CPM-graded `gross` and `microscopic` patients (their source text was `x`/`present`/`yes`/`minimal`/`focal`, none of which the keyword list recognized). Three-part fix in `scripts/266b_per_tumor_ajcc_buildout.py`: (a) add `minimal`, `focal`, `microscopiic` to `MICRO_KEYWORDS`; (b) `extensive`/`extesive`/`c/a` deliberately STAY unclassified (ambiguous); (c) NEW Tier 4 — when STL classifier returns `unclassified_present` AND tumor is dominant (largest `size_greatest_dimension_cm`) AND CPM `ete_grade_final_v2` is in `{gross, microscopic, absent}`, broadcast CPM grade with `ete_source = cpm_patient_level:broadcast_to_dominant:<grade>`. Multifocal secondary tumors stay unclassified (preserves the original broadcast-bug protection). Result: `unclassified_present` 8,706 -> 930 (-89%); CPM-gross coverage 1% -> 98%; CPM-microscopic coverage 2% -> 98.7%; AJCC8 T3b 8 -> 197 per tumor (the +189 dominant tumors correctly upstaged from CPM-gross broadcast).
- **266b pre-apply ETE gate (Logan-accepted 2026-04-17):** the dominant-tumor metric (0/16 = 0% CPM-says-absent dominants in `unclassified_present`) is the right reading post-Tier-4 (PROCEED). The any-tumor strict metric (4/16 = 25%) flags multifocal patients whose dominant is correct but secondary tumors carry ungraded source text (`x`/`present`/etc.) — known residual, not a vocabulary failure, not a blocker.
- **Secondary-tumor ungraded STL text (known residual, 2026-04-17):** ~930 secondary tumors of multifocal patients have `extrathyroidal_extension` text that is genuinely ungraded (`x`, `present`, `yes`, `c/a`, `indeterminate`, `extensive`, NaN) and cannot be classified without external info. 266b leaves these as `stl_per_tumor:unclassified_present`; downstream consumers should treat as "ETE present, grade unknown for this secondary tumor" — not as evidence of contradictory ETE relative to the dominant tumor's clinical grade.
- **STL placeholder rows (2026-04-17 audit):** 45.13% of `synoptic_tumor_long_v1` rows have NULL `size_greatest_dimension_cm`; clustering shows tumor_1_ slot at 54% NULL vs tumor_2-5 at 2-5%. This is workbook design (placeholder slot rows for benign / metastatic-only / no-thyroid-tumor patients in `All Diagnoses & synoptic 12_1_2025.xlsx`), NOT extraction failure. 266b script 266b filters via Fix A: `WHERE size_greatest_dimension_cm IS NOT NULL OR histologic_type IS NOT NULL OR extrathyroidal_extension IS NOT NULL OR margin_status IS NOT NULL`. Future scripts that consume STL should apply the same filter.
- **CPM `ete_grade_final_v2` aggregation does NOT read `operative_episode_detail_v2.tracheal_involvement_flag` as an input source** (discovered 2026-04-17 by 266b apply Phase 3 overshoot probe). 266b's per-tumor staging discovered 6 patients with OED `tracheal_involvement_flag=TRUE` who are NOT flagged gross at CPM level (rids 8388, 3328, 8535, 9012, 8616, 8254). These are correctly T4a per AJCC 8 (trachea invasion = T4a regardless of CPM ETE grade semantics). CPM's patient-level ETE should be re-aggregated in a 267-series script to pull OED substrate flags. **266b's per-tumor grain is the more granular source of truth for AJCC8 T-stage going forward.**
- **AJCC8 T4a/T4b detection limits (2026-04-17):** 266b detects T4a/T4b from (a) STL `extrathyroidal_extension` substring keywords (trachea/cricoid/esophag/RLN/prevertebral/carotid/etc.) and (b) OED `tracheal_involvement_flag` / `esophageal_involvement_flag`. STL text contains 0 substrate-specific mentions in current cohort. OED has 9 patients with tracheal_flag, 0 with esophag_flag. No prevertebral/carotid signal source exists. Detection: 7 T4a patients, 0 T4b. AJCC 8 epidemiology suggests ~20-45 T4a expected in a 4,441-patient malignant cohort; current detection likely underestimates true prevalence due to limited substrate-specific source capture. Future enhancement: NLP extraction of path report substrate descriptions to supplement OED flags. Tagged for 268-series.
- **CPM ETE self-contradiction queue:** `manuscript_workspace.cpm_ete_self_contradiction_queue_v1` holds patients where CPM ETE fields disagree internally (e.g., `ete_grade_final_v2='microscopic'` AND `gross_ete_flag=TRUE`). Initial population: rid 8254 (inserted 2026-04-17 by 266b apply). Pattern parallels `cpm_hypopara_adjudication_queue_v1`. Never auto-resolve; require manual review.
- **266b staging-frame fan-out & dedup (2026-04-17 — Bug 4 fix):** `ln_master_rollup_v1` has 4,273 rows / 3,986 distinct rids (256 rids with 2-4 rows each), causing the 266b per_tumor staging frame to fan out to 18,633 rows for an 11,106-row CTC table. Without dedup, DuckDB's `UPDATE FROM` non-deterministically picks one staging row per CTC key — could be the dominant-marked row (CPM-overlay broadcast applied) or a non-dominant twin (no broadcast). Effect on T3b assignment: non-deterministic 156-188 T3b at per-PATIENT dominant grain. **Fix: dedup staging frame to 1 row per (rid, surg_ep, tumor_ord) BEFORE UPDATE FROM, with priority `is_dominant_tumor=TRUE > classified > calculable > stable index`.** Post-dedup grain count must equal distinct-CTC-key count (10,443) — assert in code. Apply this dedup pattern to ANY future per-tumor pipeline that joins ln_master_rollup_v1 or other multi-row-per-rid tables.
- **266b Phase 3 verification (2026-04-17 commit applied):** four reporting grains for AJCC8 T-stage:
  - Staging frame (post-dedup): 10,443 rows; T3b=190, T4a=12
