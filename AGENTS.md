...
- `pd.NA` from DuckDB nullable boolean columns raises `NAType.__bool__` in Streamlit; use `val is True` pattern (not `if val:` or `if val else`) for boolean flag checks from DuckDB query results; also applies to `h.get(key) is True` in eligibility badge logic
