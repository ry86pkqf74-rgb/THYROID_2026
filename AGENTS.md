...
- MotherDuck RW token for local `--md` runs: store in **gitignored** `.streamlit/secrets.toml` under `MOTHERDUCK_TOKEN` and/or `MD_SA_TOKEN` (user convention: TOML, not only shell env). `motherduck_client.get_token()` falls back to that file when env vars are unset; never print or commit token values — logs: SET/MISSING/length only
