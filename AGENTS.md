...
- Complex multi-CTE SQL on MotherDuck can hang indefinitely (10+ min) even for moderate data sizes; prefer pull-to-pandas + compute locally + push-via-parquet strategy for any SQL with >5 CTEs or >3 JOINs against MotherDuck
- MotherDuck transaction locks from long-running CREATE TABLE statements persist even after the Python process is killed; the only resolution is to wait (can be 15+ min) or write to an alternative table name and create a VIEW alias
...
- Script 51 original SQL has multiple schema mismatches vs MotherDuck reality: `recurrence_confirmed` (should be `recurrence_flag_structured`/`recurrence_any`), `bethesda_confidence` (should be `best_source_reliability`), `bethesda_final` is BIGINT not VARCHAR, `histology_normalized` does not exist in mcv12, `surg_date` needs TRY_CAST for VARCHAR dates, `LIKE ANY (...)` requires square brackets `[...]` in DuckDB
...
- `extracted_recurrence_refined_v1` on MotherDuck has `recurrence_flag_structured`, `recurrence_any` (booleans), `recurrence_site_inferred`, `detection_category` — NOT `recurrence_confirmed`, `recurrence_site`, `recurrence_detection_method`
- `extracted_fna_bethesda_v1` on MotherDuck: `bethesda_final` is BIGINT (1-6 numeric), `best_source_reliability` (not `bethesda_confidence`), `source_tables` (not `bethesda_source`); CASE WHEN comparisons must use integers not strings
...
- When writing pandas DataFrames to MotherDuck, use parquet intermediary: `df.to_parquet(tmp); con.execute(f"CREATE TABLE x AS SELECT * FROM read_parquet('{tmp}')")` — avoids transaction conflicts from `con.register()` approach
...
- Analysis views on MotherDuck: `analysis_patient_v1` (VIEW → patient_analysis_resolved_v1), `analysis_episode_v1` (VIEW → episode_analysis_resolved_v1_dedup), `analysis_lesion_v1` (VIEW → lesion_analysis_resolved_v1)
...
- `imaging_fna_linkage_v3` has 0 rows on MotherDuck — imaging nodule size data not populated in structured tables; `imaging_nodule_long_v2.linked_fna_episode_id` cannot be backfilled until upstream imaging data is resolved
...
- MATERIALIZATION_MAP now includes `episode_analysis_resolved_v1_dedup` and `manuscript_cohort_v1` (added 2026-03-13); standard `scripts/26_motherduck_materialize_v2.py --md` refresh path covers the full resolved+manuscript layer
...
- `operative_episode_detail_v2` new columns (script 76 Phase A): `parathyroid_identified_count`, `parathyroid_resection_flag`, `frozen_section_flag`, `berry_ligament_flag`, `ebl_ml_nlp`, `op_enrichment_source` — all structurally in place but require V2 extractor output materialization to MotherDuck to populate
- `note_entities_procedures` on MotherDuck has only procedure-level entities (hemithyroidectomy, total_thyroidectomy, etc.); V2 OperativeDetailExtractor domain entities (parathyroid_management, specimen_detail, berry_ligament, etc.) are NOT materialized there — must run V2 extractors and upload to MotherDuck
...
- MotherDuck environments: `thyroid_research_2026` (prod), `thyroid_research_2026_dev` (dev), `thyroid_research_2026_qa` (qa); dev has only audit tables from script 97; prod has all 18+ episode linkage tables and 130+ other tables
- `.streamlit/secrets.toml` does NOT exist at workspace path; MOTHERDUCK_TOKEN must be obtained from environment variable only (exported in `~/.zshrc`); background terminal sessions need explicit `export MOTHERDUCK_TOKEN="..."` since they don't source `.zshrc`
- Script 99 (`99_comprehensive_final_verification.py`): 5-workstream comprehensive verification against MotherDuck prod; creates `val_multi_surgery_cohort_v3` and `val_multi_surgery_review_queue_v3` tables; outputs 5 docs + 3 export dirs; supports `--md` (default), `--local`, `--dry-run`
- Script 99 canonical truth metrics (2026-03-15): 10,871 patients, 4,136 cancer, 9,368 dedup episodes, 761 multi-surgery (1,576 episodes), 1,986 recurrence, 1,857 RAI episodes, 629 MotherDuck tables, 39,961 canonical lab rows, RAI dose 41.0%, BRAF=546, RAS=292, TERT=96, TIRADS=3,474
...
- Script 99 multi-surgery review queue: 334 medium-priority entries (patients with 2+ surgeries and potential mislinks); stored in `val_multi_surgery_review_queue_v3` on MotherDuck prod
...
- Script 100 MotherDuck workspace mode: uses `thyroid_research_2026.main.` prefix for reading prod source tables in dev/qa; UPDATEs are prod-only (`--env prod`) and target bare table names; never uses ATTACH
...
- Script 100 output tables (8): `episode_linkage_v2_sp_rescored`, `episode_linkage_v2_preop_rescored`, `episode_linkage_v2_rai_rescored`, `episode_linkage_v2_downstream_sync`, `val_episode_linkage_v2_scorecard`, `val_episode_linkage_v2_review_queue`, `val_episode_linkage_v2_nonregression`, `val_episode_linkage_v2_delta`; all mirrored as `md_*` on MotherDuck
...
- Script 101 (`101_multi_episode_linkage_hardening.py`): multi-episode linkage hardening for multi-surgery patients; 3 SQL phases (linkage quality, cross-episode contamination, ambiguity review); supports `--md`, `--dry-run`; creates 3 tables + 3 md_ mirrors on MotherDuck; exports CSV+Parquet+manifest to `exports/multi_episode_linkage_hardening_YYYYMMDD_HHMM/`; report to `docs/multi_episode_linkage_hardening_YYYYMMDD.md`
...
- `multi_surgery_episode_cohort_v1` boolean columns (`is_completion`, `reop_flag`) stored as VARCHAR text in MotherDuck (can be empty string ''); must use safe CAST via `CASE WHEN LOWER(CAST(col AS VARCHAR)) IN ('true','1','yes','x') THEN TRUE ELSE FALSE END`
...
- `val_operative_truth_state_v1` (39 rows) and `val_operative_truth_state_delta_v1` (39 rows) on MotherDuck: per-field truth-state distributions and before/after delta across 4 tables
...
- DuckDB/MotherDuck UPDATE rowcount returns -1 (safe_exec converts to 0); use Phase G delta validation tables to verify actual changes, not in-flight rowcount
- Script 105 (`105_manuscript_freeze_v1.py`): 7-phase manuscript publication freeze — environment verify, table inventory, Parquet/CSV export, checksum, manifest, optional MotherDuck stamp, integrity verify; supports `--md`, `--dry-run`, `--stamp`, `--version TAG`, `--skip-data`
