...
**Footnote (mig_265 — PMH / social history NLP coverage).** Smoking status, family history of thyroid cancer, family history of other cancers, and related social- and family-history constructs in this database rely on NLP extraction of clinical documentation. Validation round 6 documented cohort coverage far below expected community rates for these variables (on the order of sub‑percent prevalence for smoking and family history in the full surgical cohort). Descriptive or stratified reporting of these fields should be interpreted as structurally incomplete until a dedicated social-history / family-history extraction pass is completed (`CF-mig265-NLP-SOCHX-FAMHX-REFRESH-SCOPE`). Per-phenotype rollup “definitive” flags in `canonical_pmh_patient_rollup_v1` were aligned with `probable` + literal `definitive` evidence strength in migration `mig_265` (script 365).
...
**Staging.** AJCC 8th edition T, N, M, and stage group fields were resolved per institutional staging algorithm with fallback to component-level NLP extraction. Borderline/benign-with-staging quarantine rows (n = 27, mig_229) — representing FTUMP or follicular adenoma records erroneously carrying AJCC N/M positivity — were excluded from staging aggregations. **AJCC stage group rollup (mig_263 Option B).** On `main.canonical_patient_master`, the column `ajcc8_stage_group` collapses published AJCC 8th edition labels {IVA, IVB, IVC} into a single patient-level `IVB` value (mig_266b overlay family). All distant-disease (M1) thyroid carcinomas, including ATC, MTC, and PDTC, are coded as `IVB` rather than `IVC` at this rollup. Textbook-published IVA/IVB/IVC granularity remains available in `ajcc8_stage_group_resolved` for substudies that require pre-collapse notation. Descriptive stage counts in this manuscript that show IVA/IVB/IVC separately reflect the analytic tables’ staging provenance for the malignant cohort and should be read alongside this canonical-spine convention when joining back to `canonical_patient_master`.

**NLP vs structured pathology coverage (mig_266 declaration; mig_265 / CF-mig260b,c,d / mig_261c,d,e).** Vascular invasion mentioned in NLP (`nlp_path_vasc_inv_mentioned`) misses **749** patients flagged vasc-positive in `canonical_invasion_events_v1` (CAP-template patterns recovered in mig_177/179 that NLP missed). LN positivity in PMH/NLP-derived rollups disagrees with `canonical_path_malignant_events_v1.ln_involved` in **1,105** patients. Smoking status is structured for 27 of 10,871 patients (0.25%); family history of thyroid cancer (30 patients) and family history of any cancer (16 patients) extract at \<1% vs expected oncology-referral prevalence. Descriptive stratification by these constructs should cite cross-validation against `canonical_invasion_events_v1` or `canonical_path_malignant_events_v1.ln_involved` when used as analytic exposures. Social-history NLP refresh scope: `CF-mig265-NLP-SOCHX-FAMHX-REFRESH-SCOPE` (related PMH rollup “definitive” tier expanded in mig_265 to include `probable` evidence).
...
Publication-tier database: MotherDuck `thyroid_canonical_publication_v1_0` (release `pub_v1_1_20260504`)
...
Offline replication (legacy mig_230 bundle): `parquet_export/pub_v1_0_20260430/` with manifest `_MANIFEST.md` — verify alignment to `pub_v1_1_20260504` before citing as current
...
**Locked numbers refresh (2026-05-07, post–mig_086):** `M032_locked_numbers_20260507.json` — `n_recurrence` updated from 502 → 514 (+2.4%) after extraction completeness; headline metrics recomputed from **BigQuery** `thyroid-canonical-pub-2026.pub_workspace.cohort_m032_descriptive_25yr_v1` (MotherDuck cross-check identical). All other headline denominators unchanged vs 20260504 lock. See `_scripts/post_mig_086_manuscript_reconciliation_20260507.md` (M032). Smoking “former” counts remain 504 (distinct from recurrence; do not conflate with legacy recurrence lock).
